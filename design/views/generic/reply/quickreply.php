...
<div id="reply_box">
    ...
    <div class="box pad">
            ...
            <tr class="colhead_dark">
                <td colspan="<?= (Users::has_avatars_enabled() ? 2 : 1) ?>">
                    <div class="float_left"><a href="#quickreplypreview">#XXXXXX</a>
                        by <strong><?= Users::format_username(G::$LoggedUser['ID'], true, true, true, true) ?></strong>
                        Just now
                    </div>
            ...
            <tr>
                <?php if (Users::has_avatars_enabled()) { ?>
                    <td class="avatar valign_top">
                        <?= Users::show_avatar(G::$LoggedUser['Avatar'], G::$LoggedUser['ID'],
        ...
        <form class="send_form center"
              name="reply"
              id="quickpostform" <?= isset($Action) ? 'action="' . $Action . '"' : '' ?>
              method="post"<?php if (!check_perms('users_mod')) { ?> onsubmit="quickpostform.submit_button.disabled = true;"<?php } ?>
              <?php if (!G::$LoggedUser['DisableAutoSave']) { ?>data-autosave-text="quickpost"<?php } ?>>
            ...
            <div class="preview_submit">
                <?php
                if (isset($SubscribeBox) && !isset($ForumID) && false === Subscriptions::has_subscribed_comments($Document,
                        $InputID)) {
                    ?>
                    <input id="subscribebox"
                           type="checkbox"

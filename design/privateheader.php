...
<head>
    ...
    <link rel="stylesheet" type="text/css"
          href="<?= PUBLIC_ROOT . STATIC_SERVER ?>styles/global.css?v=<?= filemtime(PUBLIC_ROOT.SERVER_ROOT . STATIC_SERVER . 'styles/global.css') ?>">
    <link rel="stylesheet"
          href="<?= PUBLIC_ROOT. STATIC_SERVER ?>styles/tooltipster/style.css?v=<?= filemtime(PUBLIC_ROOT.SERVER_ROOT . STATIC_SERVER . 'styles/tooltipster/style.css') ?>"
          type="text/css"
          media="screen">
    <?php
    if (empty(G::$LoggedUser['StyleURL'])) {
        if (($StyleColors = G::$Cache->get_value('stylesheet_colors')) === false) {
            ...
            while ([$StyleName, $StyleColor] = G::$DB->next_record()) {
        ...
        if ('.css' == substr(G::$LoggedUser['StyleURL'], -4)
            && empty($StyleURLInfo['query']) && empty($StyleURLInfo['fragment'])
            && (SITE_DOMAIN == $StyleURLInfo['host'])
    ...
    $Scripts = array_merge(['jquery', 'global', 'ajax.class', 'jquery.autocomplete', 'autocomplete', 'tooltipster'],
        explode(',', $JSIncludes));
    foreach ($Scripts as $Script) {
        if ('' == trim($Script)) {
            continue;
        }
        $Async = (in_array($Script, ['jquery', 'global', 'ajax.class'], true)) ? '' : 'async';
        if (($ScriptStats = G::$Cache->get_value("script_stats_$Script")) === false || $ScriptStats['mtime'] != filemtime(SERVER_ROOT . STATIC_SERVER . "functions/$Script.js")) {
            ...
            $ScriptStats['hash'] = base64_encode(hash_file(INTEGRITY_ALGO,
                SERVER_ROOT . STATIC_SERVER . "functions/$Script.js", true));
        ...
        <script src="<?= STATIC_SERVER . "functions/$Script.js?v=$ScriptStats[mtime]" ?>"
                ...
                integrity="<?= "$ScriptStats[algo]-$ScriptStats[hash]" ?>" <?= $Async ?>></script>
        <?php
    }
    ...
    foreach ($Notifications as $Type => $Notification) {
        if (NotificationsManager::SUBSCRIPTIONS === $Type) {
...
?>
<body id="<?= 'collages' == $Document ? 'collage' : $Document ?>" class="<?= ($BodyStyles ?? '') ?>">
<div id="wrapper">
    ...
    <div id="header">
        ...
        <div id="menu">
            <ul>
                ...
                <li id="nav_user" class="nav_dropdown" <?= Format::add_class($PageID, ['user', false, false], 'active',
                    true, 'id') ?>>
        ...
        <div id="userinfo">
            <ul id="userinfo_major">
                ...
                ?>
                <li id="nav_invite"
                    class="brackets<?= Format::add_class($PageID, ['user', 'invite'], 'active', false) ?>">
                ...
                <?php } ?>
                <li id="nav_staff" class="brackets<?= Format::add_class($PageID, ['staff'], 'active', false) ?>">
            ...
            <ul id="userinfo_stats">
                <li id="stats_seeding">
                    ...
                    <span class="stat tooltip"
                          title="<?= Format::get_size(G::$LoggedUser['BytesUploaded'],
                              5) ?>"><?= Format::get_size(G::$LoggedUser['BytesUploaded']) ?></span>
                </li>
                <li id="stats_leeching">
                    <a href="torrents.php?type=leeching&amp;userid=<?= G::$LoggedUser['ID'] ?>">Down</a>:
                    <span class="stat tooltip"
                          title="<?= Format::get_size(G::$LoggedUser['BytesDownloaded'],
                              5) ?>"><?= Format::get_size(G::$LoggedUser['BytesDownloaded']) ?></span>
                </li>
                <li id="stats_ratio">
                    Ratio: <span class="stat"><?= Format::get_ratio_html(G::$LoggedUser['BytesUploaded'],
                            G::$LoggedUser['BytesDownloaded']) ?></span>
                </li>
                <?php if (!empty(G::$LoggedUser['RequiredRatio']) && G::$LoggedUser['RequiredRatio'] > 0) { ?>
                    <li id="stats_required">
                        ...
                        <span class="stat tooltip"
                              title="<?= number_format(G::$LoggedUser['RequiredRatio'],
                                  5) ?>"><?= number_format(G::$LoggedUser['RequiredRatio'], 2) ?></span>
                    </li>
                <?php }
              ...
              <a href="userhistory.php?action=token_history&amp;userid=<?= G::$LoggedUser['ID'] ?>"><?= G::$LoggedUser['FLTokens'] ?></a>
                ...
                <li id="bonus_points">
                    ...
                    <span class="stat">
              <a href="store.php"><?= number_format((float) G::$LoggedUser['BonusPoints']) ?></a>
            </span>
                </li>
        ...
        <?php if (!apcu_exists('DBKEY')) { ?>
            <a id="dbcrypt"
               class="tooltip"
               href="wiki.php?action=article&amp;name=databaseencryption"
    ...
    if (check_perms('users_mod')) {
      ...
      WHERE UserID = " . G::$LoggedUser['ID']);
            if ([$SBlogReadTime] = G::$DB->next_record()) {
      ...
      SELECT MAX(Time)
      FROM staff_blog");
            [$LatestSBlogTime] = G::$DB->next_record();
            if ($LatestSBlogTime) {
    ...
    if (G::$LoggedUser['RatioWatch']) {
        $Alerts[] = '<a href="rules.php?p=ratio">Ratio Watch</a>: You have ' . time_diff(G::$LoggedUser['RatioWatchEnds'],
                3) . ' to get your ratio over your required ratio or your leeching abilities will be disabled.';
    } elseif (1 != G::$LoggedUser['CanLeech']) {
    ...
    if (check_perms('users_mod')) {
        ...
        WHERE Status = 'Unanswered'
          AND (AssignedToUser = " . G::$LoggedUser['ID'] . "
            ...
            if (FORUM_MOD == G::$LoggedUser['PermissionID']) {
                G::$DB->query("
        SELECT COUNT(ID)
        FROM staff_pm_conversations
        WHERE Status='Unanswered'
    ...
    if (check_perms('admin_reports')) {
        ...
        if (false === $NumTorrentReports) {
            G::$DB->query("
      SELECT COUNT(ID)
      FROM reportsv2
      WHERE Status = 'New'");
            [$NumTorrentReports] = G::$DB->next_record();
            G::$Cache->cache_value('num_torrent_reportsv2', $NumTorrentReports, 0);
        }
        
        $ModBar[] = '<a href="reportsv2.php">' . $NumTorrentReports . ((1 == $NumTorrentReports) ? ' Report' : ' Reports') . '</a>';
        
        // Other reports code
        $NumOtherReports = G::$Cache->get_value('num_other_reports');
        if (false === $NumOtherReports) {
            G::$DB->query("
      SELECT COUNT(ID)
      FROM reports
      WHERE Status = 'New'");
            [$NumOtherReports] = G::$DB->next_record();
            G::$Cache->cache_value('num_other_reports', $NumOtherReports, 0);
        }
        
        if ($NumOtherReports > 0) {
            $ModBar[] = '<a href="reports.php">' . $NumOtherReports . ((1 == $NumTorrentReports) ? ' Other report' : ' Other reports') . '</a>';
        }
    } elseif (check_perms('project_team')) {
        $NumUpdateReports = G::$Cache->get_value('num_update_reports');
        if (false === $NumUpdateReports) {
            G::$DB->query("
      SELECT COUNT(ID)
      FROM reports
      WHERE Status = 'New'
        AND Type = 'request_update'");
            [$NumUpdateReports] = G::$DB->next_record();

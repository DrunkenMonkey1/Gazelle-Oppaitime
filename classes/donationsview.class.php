...
class DonationsView
{
    public static function render_mod_donations($UserID): void
    {
        ?>
        ...
        <table class="layout box" id="donor_points_box">
            ...
            <tr>
                <td class="label tooltip" title="Active points determine a user's Donor Rank and do expire.">Active
                    points:
                </td>
                <td><input type="text"
                           name="donor_rank"
    ...
    public static function render_donor_stats($UserID): void
    {
            ...
            <div class="box box_info box_userinfo_donor_stats">
                ...
                <ul class="stats nobullet">
                    ...
                    if (Donations::is_donor($UserID)) {
                        ...
                        <li>
                            Current donor rank: <?= self::render_rank(Donations::get_rank($UserID),
                                Donations::get_special_rank($UserID), true) ?>
                        </li>
                        <li>
    ...
    public static function render_rank($Rank, $SpecialRank, $ShowOverflow = false): void
    {
        if (3 == $SpecialRank) {
            $Display = '∞ [Diamond]';
        } else {
    ...
    public static function render_profile_rewards($EnabledRewards, $ProfileRewards): void
    {
        for ($i = 1; $i <= 4; $i++) {
            if ($EnabledRewards['HasProfileInfo' . $i] && $ProfileRewards['ProfileInfo' . $i]) {
                ...
                <div class="box">
                    <div class="head">
                        <span><?= empty($ProfileRewards['ProfileInfoTitle' . $i]) ? "Extra Profile " . ($i + 1) : display_str($ProfileRewards['ProfileInfoTitle' . $i]) ?></span>
                        <span class="float_right"><a data-toggle-target="#profilediv_<?= $i ?>"
    ...
    public static function render_donation_history($DonationHistory): void
    {
        ...
        <div class="box box2" id="donation_history_box">
            <div class="head">
                Donation History <a data-toggle-target="#donation_history"
                                    class="brackets"
                                    style="float_right">Toggle</a>
            </div>

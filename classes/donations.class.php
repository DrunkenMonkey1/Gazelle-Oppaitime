...
class Donations
{
     ...
     * @var mixed[]
     */
    private static array $ForumDescriptions = [
    ...
    private static bool $IsSchedule = false;
    
    public static function regular_donate($UserID, $DonationAmount, $Source, $Reason, $Currency = "EUR"): void
    {
        self::donate($UserID, [
            "Source" => $Source,
    ...
    public static function donate($UserID, $Args): void
    {
        $UserID = (int) $UserID;
      ...
      LIMIT 1");
        ...
        WHERE UserID = '$UserID'");
            ...
            if (isset($Manipulation) && "Direct" === $Manipulation) {
                $DonorPoints = $Rank;
          ...
          ON DUPLICATE KEY UPDATE
            ...
            RankExpirationTime = NOW()");
                ...
                $AdjustedRank = ($CurrentRank + $DonorPoints) >= MAX_EXTRA_RANK ? MAX_EXTRA_RANK : $CurrentRank + $DonorPoints;
                G::$DB->query("
          INSERT INTO users_donor_ranks
            (UserID, Rank, TotalRank, DonationTime, RankExpirationTime)
          VALUES
          ...
          WHERE UserID = '$UserID'");
            [$InvitesRecievedRank] = G::$DB->next_record();
    ...
    public static function currency_exchange($Amount, $Currency): float
    {
    ...
    public static function is_valid_currency($Currency): bool
    {
        return 'EUR' == $Currency || 'BTC' == $Currency || 'USD' == $Currency;
    }
    
    public static function usd_to_euro($Amount): float|int
    {
    ...
    public static function get_stored_conversion_rate($Currency)
    {
      ...
      WHERE Currency = '$Currency'");
        [$Rate] = G::$DB->next_record(MYSQLI_NUM, false);
        G::$DB->set_query_id($QueryID);
        
        return $Rate;
    }
    
    public static function btc_to_euro($Amount): float|int
    {
    ...
    private static function calculate_rank($Amount): float
    {
    ...
    public static function get_donor_info($UserID)
    {
        ...
        WHERE UserID = '$UserID'");
            ...
            if (G::$DB->has_results()) {
                [$Rank, $SpecialRank, $TotalRank, $DonationTime, $ExpireTime] = G::$DB->next_record(MYSQLI_NUM, false);
                if (null === $DonationTime) {
                    $DonationTime = 0;
                }
                if (null === $ExpireTime) {
                    $ExpireTime = 0;
    ...
    private static function calculate_special_rank($UserID): void
    {
        $UserID = (int) $UserID;
        $QueryID = G::$DB->get_query_id();
      ...
      WHERE UserID = '$UserID'");
        if (G::$DB->has_results()) {
            ...
            [$TotalRank, $SpecialRank] = G::$DB->next_record();
            if ($TotalRank < 10) {
                $SpecialRank = 0;
            }
            if ($SpecialRank < 1 && $TotalRank >= 10) {
                Misc::send_pm($UserID, 0,
                    "You've Reached Special Donor Rank #1! You've Earned: One User Pick. Details Inside.",
                    self::get_special_rank_one_pm());
                $SpecialRank = 1;
            }
            if ($SpecialRank < 2 && $TotalRank >= 20) {
                Misc::send_pm($UserID, 0,
                    "You've Reached Special Donor Rank #2! You've Earned: The Double-Avatar. Details Inside.",
                    self::get_special_rank_two_pm());
                $SpecialRank = 2;
            }
            if ($SpecialRank < 3 && $TotalRank >= 50) {
                Misc::send_pm($UserID, 0,
                    "You've Reached Special Donor Rank #3! You've Earned: Diamond Rank. Details Inside.",
                    self::get_special_rank_three_pm());
                $SpecialRank = 3;
            }
            // Make them special
            G::$DB->query("
    ...
    private static function get_special_rank_one_pm(): string
    {
...
' . SITE_NAME . ' Staff';
    ...
    private static function get_special_rank_two_pm(): string
    {
...
' . SITE_NAME . ' Staff';
    ...
    private static function get_special_rank_three_pm(): string
    {
...
' . SITE_NAME . ' Staff';
    ...
    private static function get_pm_body($Source, $Currency, $DonationAmount, $ReceivedRank, $CurrentRank): string
    {
        if ('BTC' != $Currency) {
...
[*][b]You Received:[/b] $ReceivedRank Donor Point" . (1 == $ReceivedRank ? '' : 's') . "
...
[align=center][If you have any questions or concerns, please [url=' . site_url() . 'staffpm.php]send a Staff PM[/url].]';
    ...
    public static function expire_ranks(): void
    {
        $QueryID = G::$DB->get_query_id();
        G::$DB->query("
      SELECT UserID, Rank
      ...
      WHERE Rank > 1
        ...
        if (G::$DB->record_count() > 0) {
            $UserIDs = [];
            while ([$UserID, $Rank] = G::$DB->next_record()) {
                G::$Cache->delete_value("donor_info_$UserID");
    ...
    private static function get_new_conversion_rates(): void
    {
        if ($BTC = file_get_contents(BTC_API_URL)) {
            $BTC = json_decode($BTC, true);
            if (isset($BTC['24h_avg']) && ($Rate = round($BTC['24h_avg'], 4))) {
                // We don't need good precision
        ...
        if ($USD = file_get_contents(USD_API_URL)) {
            ...
            if (isset($USD['rhs'])) {
                ...
                if (($Rate = round($Rate[0], 4)) !== 0.0) { // We don't need good precision
    ...
    private static function set_stored_conversion_rate($Currency, $Rate): void
    {
        $QueryID = G::$DB->get_query_id();
        G::$DB->query("
    ...
    public static function update_rank($UserID, $Rank, $TotalRank, $Reason): void
    {
        $Rank = (int) $Rank;
    ...
    public static function hide_stats($UserID): void
    {
        $QueryID = G::$DB->get_query_id();
        G::$DB->query("
      INSERT INTO users_donor_ranks
    ...
    public static function show_stats($UserID): void
    {
        $QueryID = G::$DB->get_query_id();
        G::$DB->query("
      INSERT INTO users_donor_ranks
    ...
    public static function has_donor_forum($UserID): bool
    {
    ...
    public static function get_personal_collages($UserID)
    {
        ...
        if (MAX_SPECIAL_RANK == $DonorInfo['SRank']) {
            $Collages = 5;
        } else {
     ...
     */
    public static function get_enabled_rewards($UserID): array
    {
        $Rewards = [];
        $Rank = self::get_rank($UserID);
        $SpecialRank = self::get_special_rank($UserID);
        $HasAll = 3 == $SpecialRank;
...
//    }
    ...
    public static function update_rewards($UserID): void
    {
        $Rank = self::get_rank($UserID);
        $SpecialRank = self::get_special_rank($UserID);
        $HasAll = 3 == $SpecialRank;
        ...
        if ($Rank >= MAX_RANK || $HasAll) {
            ...
            self::update_titles($UserID, $_POST['donor_title_prefix'], $_POST['donor_title_suffix'],
                $_POST['donor_title_comma']);
            $Counter++;
        }
        for ($i = 1; $i <= $Counter; $i++) {
            self::add_profile_info_reward($i, $Insert, $Values, $Update);
        }
        if ($SpecialRank >= 2 && isset($_POST['second_avatar'])) {
    ...
    public static function update_titles($UserID, $Prefix, $Suffix, $UseComma): void
    {
        $QueryID = G::$DB->get_query_id();
    ...
    private static function add_profile_info_reward($Counter, &$Insert, &$Values, &$Update): void
    {
    ...
    public static function get_rank_expiration($UserID): string
    {
        $DonorInfo = self::get_donor_info($UserID);
        if (MAX_SPECIAL_RANK == $DonorInfo['SRank'] || 1 == $DonorInfo['Rank']) {
            $Return = 'Never';
    ...
    public static function is_donor($UserID): bool
    {

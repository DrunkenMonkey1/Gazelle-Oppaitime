...
class Users
{
    ...
    public static function get_classes(): array
    {
        global $Debug;
        // Get permissions
        [$Classes, $ClassLevels] = G::$Cache->get_value('classes');
     ...
     */
    public static function update_site_options(int $UserID, array $NewOptions): bool
    {
        if (!is_number($UserID)) {
            error((string) 0);
        }
      ...
      FROM users_info
      WHERE UserID = $UserID");
        [$SiteOptions] = G::$DB->next_record(MYSQLI_NUM, false);
        ...
        G::$DB->query("
      UPDATE users_info
      SET SiteOptions = '" . db_string(json_encode($SiteOptions)) . "'
      WHERE UserID = $UserID");
        G::$DB->set_query_id($QueryID);
        ...
        WHERE UserID = $UserID");
            ...
            foreach ($PermIDs as $PermID) {
                ...
                if (!empty($Perms['PermittedForums'])) {
                    $PermittedForums = [
                        ...$PermittedForums,
                        ...array_map('trim', explode(',', $Perms['PermittedForums']))
                    ];
                }
            }
            ...
            if (!empty($Perms['PermittedForums'])) {
                $PermittedForums = [...$PermittedForums, ...array_map('trim', explode(',', $Perms['PermittedForums']))];
            }
            ...
            if (isset($HeavyInfo['SiteOptions'])) {
     ...
     */
    public static function make_sec_hash(string $Str): string
    {
        return password_hash($Str, PASSWORD_ARGON2ID, PASSWORD_HASH_OPTIONS);
    }
     ...
     *
     * @param      $UserID
     * @param bool $Badges       whether or not badges (donor, warned, enabled) should be shown
     * @param bool $IsWarned
     * @param bool $IsEnabled
     * @param bool $Class        whether or not to show the class
     * @param bool $Title        whether or not to show the title
     * @param bool $IsDonorForum for displaying donor forum honorific prefixes and suffixes
     *
     * @return string formatted username
     * @throws \JsonException
     */
    public static function format_username(
        $UserID,
        bool $Badges = false,
        bool $IsWarned = true,
        bool $IsEnabled = true,
        ...
        $ShowDonorIcon = (!in_array('hide_donor_heart', $Paranoia, true) || $OverrideParanoia);
        
        if ($IsDonorForum) {
            [$Prefix, $Suffix, $HasComma] = Donations::get_titles($UserID);
        ...
        if ($Badges) {
            ...
            if (0 == $DonorRank && 1 == $UserInfo['Donor']) {
                $DonorRank = 1;
            }
            if ($ShowDonorIcon && $DonorRank > 0) {
                ...
                if ($EnabledRewards['HasCustomDonorIcon'] && !empty($DonorRewards['CustomIcon'])) {
                    ...
                    if (MAX_SPECIAL_RANK === $SpecialRank) {
                        $DonorHeart = 6;
                    } elseif (5 === $DonorRank) {
        ...
        $Str .= ($IsWarned && $UserInfo['Warned']) ? '<a href="wiki.php?action=article&amp;name=warnings"'
            ...
            . (G::$LoggedUser['ID'] === $UserID ? ' - Expires ' . date('Y-m-d H:i',
                    strtotime($UserInfo['Warned'])) : '')
            . '" class="tooltip" /></a>' : '';
        $Str .= ($IsEnabled && 2 == $UserInfo['Enabled']) ? '<a href="rules.php"><img src="' . STATIC_SERVER . 'common/symbols/disabled.png" alt="Banned" title="Disabled" class="tooltip" /></a>' : '';
     ...
     * @throws \JsonException
     */
    public static function user_info($UserID): array
    {
        global $Classes;
        ...
        GROUP BY m.ID");
            ...
            if (!G::$DB->has_results()) { // Deleted user, maybe?
                ...
                if (isset($UserInfo['CatchupTime'])) {
                    $UserInfo['CatchupTime'] = strtotime($UserInfo['CatchupTime']);
                }
                if (!is_array($UserInfo['Paranoia']) && isset($UserInfo['Paranoia'])) {
                    $UserInfo['Paranoia'] = json_decode($UserInfo['Paranoia'], true, 512, JSON_THROW_ON_ERROR);
                }
          ...
          WHERE UserID = " . $UserID);
                ...
                $Badges = [];
                if (false !== G::$DB->has_results()) {
                    while ([$BadgeID, $Displayed] = G::$DB->next_record()) {
            ...
            G::$Cache->cache_value("user_info_$UserID", $UserInfo, 2_592_000);
            G::$DB->set_query_id($OldQueryID);
        }
        if (isset($UserInfo['Warned']) && strtotime($UserInfo['Warned']) < time()) {
    ...
    public static function make_class_abbrev_string($ClassID): string
    {
        global $Classes;
     ...
     * @return string name
     */
    public static function make_class_string(int $ClassID): string
    {
        global $Classes;
     ...
     */
    public static function get_bookmarks(string|int $UserID): array
    {
        $UserID = (int) $UserID;
        
        if (($Data = G::$Cache->get_value("bookmarks_group_ids_$UserID"))) {
            [$GroupIDs, $BookmarkData] = $Data;
        } else {
            $QueryID = G::$DB->get_query_id();
            G::$DB->query("
     ...
     * @return string
     */
    public static function show_avatar(
        $Avatar,
        $UserID,
        $Username,
        $Setting,
        ...
        switch ($Setting) {
            ...
            case 3:
                ...
                if (true == $ShowAvatar && !empty($Avatar)) {
    ...
    public static function has_avatars_enabled(): bool
    {
        global $HeavyInfo;
        
        return isset($HeavyInfo['DisableAvatars']) && (1 !== $HeavyInfo['DisableAvatars']);
    }
     ...
     * @param bool   $Output echo out HTML
     *
     * @return void|bool
     */
    public static function has_autocomplete_enabled(string $Type, bool $Output = true)
    {
        $Enabled = false;
        if (empty(G::$LoggedUser['AutoComplete'])) {
            $Enabled = true;
        } elseif (1 !== G::$LoggedUser['AutoComplete']) {
            switch ($Type) {
                case 'search':
                    if (2 == G::$LoggedUser['AutoComplete']) {
                        $Enabled = true;
                    }
                    break;
                case 'other':
    ...
    public static function reset_password($UserID, $Username, $Email): void
    {
     ...
     */
    public static function make_secret($Length = 32): string
    {
        if (false !== empty(SECRET_KEY) && false !== is_string(SECRET_KEY)) {
            $Chars = SECRET_KEY;
        } else {
    ...
    public static function auth_location($UserID, $Username, $ASN, $Email): void
    {
    ...
    public static function get_upload_sources(): array
    {
      ...
      WHERE UserID = " . G::$LoggedUser['ID']);
        ...
        $Source[1] = $SourceKeyOld ? SITE_NAME . '-' . substr(hash('sha256',
                $SourceKeyOld[0] . G::$LoggedUser['ID'] . $Uploads), 0, 10) : $Source[0];

...
class Text
{
     ...
     * @var array $ValidTags
     */
    private static $ValidTags = ['b'=>0, 'u'=>0, 'i'=>0, 's'=>0, '*'=>0, '#'=>0, 'ch'=>0, 'uch'=>0, 'artist'=>0, 'user'=>0, 'n'=>0, 'inlineurl'=>0, 'inlinesize'=>1, 'headline'=>1, 'align'=>1, 'color'=>1, 'colour'=>1, 'size'=>1, 'url'=>1, 'img'=>1, 'quote'=>1, 'pre'=>1, 'code'=>1, 'tex'=>0, 'hide'=>1, 'spoiler' => 1, 'plain'=>0, 'important'=>0, 'torrent'=>0, 'rule'=>0, 'embed'=>0,
    ];
    ...
    */
    ...
    private static function to_html($Array)
    {
        ...
        case 'colour':
          ...
          if (!in_array($Block['Attr'], $ValidAttribs, true) && !preg_match('/^#[0-9a-f]{6}$/', $Block['Attr'])) {
        ...
        case 'quote':
          ...
          if (!empty($Block['Attr'])) {
              ...
              if (isset($Exploded[1]) && (is_numeric($Exploded[1]) || (in_array($Exploded[1][0], ['a', 't', 'c', 'r'], true) && is_numeric(substr($Exploded[1], 1))))) {
    ...
    private static function smileys($Str)
    {
        ...
        if (0 == count(self::$ProcessedSmileys) && count(self::$Smileys) > 0) {

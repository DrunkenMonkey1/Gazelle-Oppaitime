...
class Text
{
     ...
     */
    private static ?array $Headlines = null;
    /**
     ...
     */
    public static function full_format($Str, bool $OutputTOC = true, int $Min = 3): string
    {
        if (null === $Str) {
            return '';
        }
        ...
        $Str = preg_replace_callback('/(?<=\[inlineurl\]|' . $URLPrefix . ')(\S*\[inlineurl\]\S*)/m',
            fn ($matches) => str_replace("[inlineurl]", "", $matches[0]), $Str);
            ...
            if (self::$Levels < self::$MaximumNests) {
                switch ($Block['Type']) {
                    ...
                    case 'rule':
                        ...
                        if ('r' != $Rule[0] && 'h' != $Rule[0]) {
                            $Rule = 'r' . $Rule;
                        }
                        $Str .= '<a href="rules.php?p=upload#' . urlencode(Format::undisplay_str($Rule)) . '">' . preg_replace('/[aA-zZ]/',
                                '', $Block['Val']) . '</a>';
                        break;
                    case 'torrent':
                            ...
                            if (isset($Matches[2])) {
                                ...
                                if ($Groups[$GroupID]) {
                                    ...
                                    if (!isset($LoggedUser['CoverArt']) || $LoggedUser['CoverArt']) {
                                        $Str .= ' data-cover="' . ImageTools::process($Group['WikiImage'],
                                                'thumb') . '"';
                                    }
                                    $Name = $Group['Name'];
                                    $Str .= '>' . $Name . '</a>';
                    ...
                    case 'wiki':
                        ...
                        break;
                    case 'tex':
                        $Str .= '<img class="tex_img" style="vertical-align: middle;" src="' . STATIC_SERVER . 'blank.gif" onload="if (this.src.substr(this.src.length - 9, this.src.length) == \'blank.gif\') { this.src = \'' . ImageTools::process('https://chart.googleapis.com/chart?cht=tx&chf=bg,s,FFFFFF00&chl=' . urlencode(mb_convert_encoding($Block['Val'],
                                    'UTF-8', 'HTML-ENTITIES'))) . '\'; }" alt="' . $Block['Val'] . '" />';
                        break;
                    case 'plain':
                        $Str .= $Block['Val'];
                        break;
                    ...
                    case 'align':
                        ...
                        if (!in_array($Block['Attr'], $ValidAttribs, true)) {
                    ...
                    case 'colour':
                        ...
                        if (!in_array($Block['Attr'], $ValidAttribs, true) && !preg_match('/^#[0-9a-f]{6}$/',
                                $Block['Attr'])) {
                    ...
                    case 'headline':
                        ...
                        if (!in_array($Block['Attr'], self::$HeadlineLevels, true)) {
                    ...
                    case 'quote':
                        ...
                        if (!empty($Block['Attr'])) {
                            ...
                            if (isset($Exploded[1]) && (is_numeric($Exploded[1]) || (in_array($Exploded[1][0],
                                            ['a', 't', 'c', 'r'], true) && is_numeric(substr($Exploded[1], 1))))) {
                    ...
                    case 'embed':
                        ...
                        if (self::valid_url($Val) && 'webm' == substr($Val, -4)) {
    ...
    private static function smileys($Str)
    {
        ...
        if (0 == count(self::$ProcessedSmileys) && [] !== self::$Smileys) {
    ...
    private static function raw_text($Array)
    {
        ...
        foreach ($Array as $Block) {
            ...
            switch ($Block['Type']) {
                ...
                case 'url':
                    $Block['Val'] = empty($Block['Val']) ? $Block['Attr'] : self::raw_text($Block['Val']);
    ...
    */
    ...
    private static function valid_url($Str, $Extension = '', $Inline = false): int|bool
    {
        $Regex = '/^';
     ...
     * @return mixed[]
     */
    private static function parse($Str): array
    {
        ...
        while ($i < $Len) {
            ...
            if (!empty($Tag[4][0])) { // Wiki-link
                ...
                $Attrib = empty($Tag[3][0]) ? '' : substr($Tag[3][0], 1);
            }
            ...
            //5a) Different for different types of tag. Some tags don't close, others are weird like [*]
            if ('img' == $TagName && !empty($Tag[3][0])) { //[img=...]
                $Block = ''; // Nothing inside this tag
                // Don't need to touch $i
            } elseif ('inlineurl' == $TagName) { // We did a big replace early on to turn http:// into [inlineurl]http://
                
                // Let's say the block can stop at a newline or a space
                $CloseTag = strcspn($Str, " \n\r", $i);
                if (false === $CloseTag) { // block finishes with URL
                    $CloseTag = $Len;
                }
            ...
            } elseif (true == $WikiLink || 'n' == $TagName) {
                ...
                do { // Look for \n[*]
                    $NewLine = strpos($Str, "\n", $NewLine + 1);
                } while (false !== $NewLine && substr($Str, $NewLine + 1, 3) == "[$TagName]");
                
                $CloseTag = $NewLine;
                if (false === $CloseTag) { // block finishes with list
                    $CloseTag = $Len;
                }
                ...
                $Block = substr($Str, $i,
                    $CloseTag - $i); // 5c) Get the contents between [open] and [/close] and call it the block.
            ...
            switch ($TagName) {
                ...
                case 'hide':
                    ...
                    break;
                case 'embed':
                    $Array[$ArrayPos] = ['Type' => 'embed', 'Val' => $Block];
                    break;
                case '#':
                case '*':
    ...
    public static function local_url($Str)
    {
        ...
        if (SITE_DOMAIN == $Host || $Host === 'www.' . SITE_DOMAIN) {
     ...
     * @return string|void
     */
    public static function parse_toc(int $Min = 3)
    {
     ...
     */
    private static function headline_level(int &$ItemLevel, int &$Level, string &$List, int $i, int &$Offset): void
    {
        if ($ItemLevel < $Level) {
    ...
    public static function strip_bbcode($Str): string
    {
        $Str = display_str($Str);

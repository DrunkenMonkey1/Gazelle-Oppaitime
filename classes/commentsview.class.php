...
class CommentsView
{
     ...
     */
    public static function render_comments(array $Thread, bool|int $LastRead, string $Baselink): void
    {
        foreach ($Thread as $Post) {
            ...
            self::render_comment($AuthorID, $PostID, $CommentBody, $AddedTime, $EditedUserID, $EditedTime,
                $Baselink . "&amp;postid=$PostID#post$PostID", ($PostID > $LastRead));
        }
    }
     ...
     *
     * @todo Find a better way to pass the page (artist, collages, requests, torrents) to this function than extracting
     *       it from $Link
     */
    public static function render_comment(
        int $AuthorID,
        int $PostID,
        string $Body,
        string $AddedTime,
        ...
        <table class="forum_post box vertical_margin<?= (Users::has_avatars_enabled() ? '' : ' noavatar') . ($Unread ? ' forum_unread' : '') ?>"
               id="post<?= $PostID ?>">
            <colgroup>
            ...
            <tr class="colhead_dark">
                <td colspan="<?= (Users::has_avatars_enabled() ? 2 : 1) ?>">
                    <div class="float_left"><a class="post_id" href="<?= $Link ?>">#<?= $PostID ?></a>
                        ...
                        $Header = Users::format_username($AuthorID, true, true, true, true, true) .
                        time_diff($AddedTime) . $Header;<?= $Header ?>
                        <?php if ($Tools) { ?>
                        - <a href="#quickpost"
            ...
            <tr>
                <?php if (Users::has_avatars_enabled()) { ?>
                    <td class="avatar" valign="top">
                        <?= Users::show_avatar($UserInfo['Avatar'], $AuthorID, $UserInfo['Username'],
                            G::$LoggedUser['DisableAvatars']) ?>
                    </td>
                <?php } ?>
                <td class="body" valign="top">
                    <div id="content<?= $PostID ?>">
                        <?= Text::full_format($Body) ?>
                        <?php if (0 !== $EditedUserID) { ?>
                        <br/>
                        <br/>
                        <div class="last_edited">
                            <?php if (check_perms('site_admin_forums')) { ?>
                                <a href="#content<?= $PostID ?>"
                                   onclick="LoadEdit('<?= substr($Link, 0,
                                       strcspn($Link, '.')) ?>', <?= $PostID ?>, 1); return false;">&laquo;</a>
                            <?php } ?>
                            Last edited by

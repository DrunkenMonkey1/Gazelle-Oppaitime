...
class CalendarView
{
     ...
     * @var mixed[]
     */
    private static array $Days = ['S', 'M', 'T', 'W', 'T', 'F', 'S'];
    /**
     * @var mixed[]
     */
    private static array $Headings = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'];
    private static $Events;
    
    public static function render_title($Month, $Year): void
    {
        if (!is_numeric($Month) || !is_numeric($Year)) {
            error(404);
        }
        $NextMonth = 0 == $Month % 12 ? 1 : $Month + 1;
    ...
    public static function render_calendar($Month, $Year, $Events): void
    {
    ...
    private static function render_events_day($Day, $Events): void
    {
        ...
        foreach ($Events as $Event) {
            ...
            <p><a href="#"
                  class="event_day tooltip"
                  title="<?= $Tooltip ?>"
                  data-gazelle-id="<?= $Event['ID'] ?>"
                  style="color: <?= $Color ?>;"><?= Format::cut_string($Event['Title'], 8, true) ?></a></p>
            <?php
        }
    }
     ...
     * @return mixed[]
     */
    private static function get_events_on($Day, $Events): array
    {
        // Linear search, Lol.
        $Results = [];
        foreach ($Events as $Event) {

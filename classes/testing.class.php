...
class Testing
{
    private static $ClassDirectories = ["classes"];
    ...
    private static function get_class_name($FileName)
    {
        ...
        foreach ($Tokens as $Token) {
            if (is_array($Token)) {
                if (!$IsTestable && T_DOC_COMMENT == $Token[0] && strpos($Token[1], "@TestClass")) {
                    $IsTestable = true;
                }
                if ($IsTestable && T_CLASS == $Token[0]) {
                    $IsClass = true;
                } elseif ($IsClass && T_STRING == $Token[0]) {

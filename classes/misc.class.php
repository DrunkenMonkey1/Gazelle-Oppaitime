...
class Misc
{
     ...
     * @return bool   true if (substring of) $Needle exists in $Haystack
     */
    ...
    public static function display_array($Array, $Escape = [])
    {
        foreach ($Array as $Key => $Val) {
            if ((!is_array($Escape) && true == $Escape) || !in_array($Key, $Escape, true)) {
    ...
    {
        return (false !== array_search($Needle, explode($Separator, $Haystack), $Strict));
    }
    ...
    {
        return false !== strpos(substr($Data, 0, 10), ':');
    }

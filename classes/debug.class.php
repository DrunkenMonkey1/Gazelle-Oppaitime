...
class DEBUG
{
    ...
    public function profile($Automatic = '')
    {
        ...
        */
        ...
        if (in_array(0, $CacheStatus, true) && !G::$Cache->get_value('cache_fail_reported')) {
...
<?php
    ...
    <tr>
      <td class="debug_info debug_loggedvars_name">
        <a href="#" onclick="$('#debug_loggedvars_<?=$ID?>').gtoggle(); return false;"><?=display_str($Key)?></a> (<?=$Size . (1 == $Size ? ' element' : ' elements')?>)

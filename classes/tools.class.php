...
class Tools
{
    ...
    public static function geoip($IP)
    {
      ...
      LIMIT 1");
        if ((![$EndIP, $Country] = G::$DB->next_record()) || $EndIP < $Long) {
            $Country = '?';
        }
        G::$DB->set_query_id($QueryID);
        $IPs[$IP] = $Country;
    ...
    public static function lookup_ip(string $IP): string|bool
    {
        ...
        if (1 == count($Output) && empty($Output[0])) {
            return '';
        }
        if (5 != count($Output)) {
            return false;
    ...
    public static function disable_users(array $UserIDs, $AdminComment, $BanReason = 1): void
    {
      ...
      SET m.Enabled = '2',
        ...
        i.RatioWatchDownload = " . (2 == $BanReason ? 'm.Downloaded' : "'0'") . "
      WHERE m.ID IN(" . implode(',', $UserIDs) . ') ');
        ...
        WHERE UserID = '$UserID'
          AND Active = 1");
            while ([$SessionID] = G::$DB->next_record()) {

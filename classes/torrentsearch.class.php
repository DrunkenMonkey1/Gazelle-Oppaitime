...
class TorrentSearch
{
    ...
    public function __construct($GroupResults, $OrderBy, $OrderWay, $Page, $PageSize)
    {
        ...
        || !in_array($OrderWay, ['asc', 'desc'], true)
    ) {
            global $Debug;
    ...
    private function search_basic($Term)
    {
        ...
        foreach (explode(' ', $Term) as $Word) {
            if (in_array($Word, $SearchBitrates, true)) {
                $this->add_word('encoding', $Word);
            } elseif (in_array($Word, $SearchFormats, true)) {
                $this->add_word('format', $Word);
            } elseif (in_array($Word, $SearchMedia, true)) {
    ...
    private function add_word($Field, $Word)
    {
        ...
        if ('!' === $Word[0] && strlen($Word) >= 2 && false === strpos($Word, '!', 1)) {

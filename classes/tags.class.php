...
class Tags
{
     ...
     */
    public static function format_top($Max = 5, $Link = 'torrents.php?taglist=', $ArtistName = ''): void
    {
        if (empty(self::$All)) { ?>
        ...
        foreach (array_slice(self::sorted(), 0, $Max) as $Tag => $Total) {
            ...
            ?>
            <li><a class="<?= $Class ?>"
                   href="<?= $Link . display_str($Name) . $ArtistName ?>"><?= display_str($Name) ?></a> (<?= $Total ?>)
            </li>
        <?php }
    }
     ...
     */
    public static function tag_filter_sph(array $Tags, int $TagType)
    {
        $QueryParts = [];
     ...
     * @return array
     */
    public static function remove_aliases(array $Tags): array
    {
     ...
     */
    public function set_primary($Primary): void
    {
     ...
     */
    public function css_name(): string
    {
     ...
     */
    public function format($Link = 'torrents.php?taglist=', $ArtistName = ''): string
    {
        if (!empty($ArtistName)) {
            $ArtistName = "&amp;artistname=" . urlencode($ArtistName) . "&amp;action=advanced&amp;searchsubmit=1";
        }
    ...
    public static function get_name_and_class($Tag)
    {
        ...
        return ["name" => display_str($Name), "class" => display_str($Class)];
    }
}

...
class View
{
     ...
     */
    public static function show_header($PageTitle = '', $JSIncludes = '', $CSSIncludes = ''): void
    {
        ...
        if (!is_array(G::$LoggedUser) || empty(G::$LoggedUser['ID']) || $PageTitle === 'Recover Password :: ' . SITE_NAME) {
            ...
            $item0Unpacked = ['tooltipster'];
            $item1Unpacked = explode(',', $CSSIncludes);
            $Styles = [...$item0Unpacked, ...$item1Unpacked];
            foreach ($Scripts as $Script) {
                if ('' == trim($Script)) {
                    continue;
                }
                header('Link: <' . PUBLIC_ROOT . STATIC_SERVER . 'functions/' . $Script . '.js?v=' . filemtime(SERVER_ROOT . PUBLIC_ROOT . STATIC_SERVER . 'functions/' . $Script . '.js') . '>; rel=preload; as=script;',
                    false);
            }
            header('Link: <' . PUBLIC_ROOT . STATIC_SERVER . 'styles/global.css?v=' . filemtime(SERVER_ROOT . PUBLIC_ROOT . STATIC_SERVER . 'styles/global.css') . '>; rel=preload; as=style',
                false);
            foreach ($Styles as $Style) {
                if ('' == trim($Style)) {
                    continue;
                }
                header('Link: <' . STATIC_SERVER . 'styles/' . $Style . '/style.css?v=' . filemtime(SERVER_ROOT . PUBLIC_ROOT . STATIC_SERVER . 'styles/' . $Style . '/style.css') . '>; rel=preload; as=style;',
                    false);
            }
     ...
     */
    public static function show_footer($Options = []): void
    {
        global $ScriptStartTime, $SessionID, $UserSessions, $Debug, $Time, $Mobile;
        if (!is_array(G::$LoggedUser) || (isset($Options['recover']) && true === $Options['recover'])) {
     ...
     */
    public static function render_template(string $TemplateName, array $Args): void
    {
     ...
     * @param bool   $Buffer       enables Output Buffer
     *
     ...
     *          </pre>
     */
    public static function parse(string $TemplateFile, array $Variables = [], bool $Buffer = false): bool|int|string
    {

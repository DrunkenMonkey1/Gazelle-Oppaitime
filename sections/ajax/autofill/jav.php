...
if (!$debug && $Cache->get_value('jav_fill_json_' . $cn)) {
    ...
    [$idols, $genres, $screens, $title, $title_jp, $year, $studio, $label, $desc, $image] = [[], [], [], '', '', '', '', '', '', ''];
    ...
    if ($jlib_page_en && $jlib_en->query('//*[@id="video_title"]')['length']) {
        ...
        if (isset($screens_t[1])) {
            ...
            function f($s)
            {
                return !(preg_match('/(rapidgator)|(uploaded)|(javsecret)|(\.gif)|(google)|(thumb)|(imgur)|(fileboom)|(openload)/', $s));
            }
    ...
    $genres = array_values(array_intersect(array_values($Cache->get_value('genre_tags')), str_replace('_', '.', array_values(Tags::remove_aliases(['include' => str_replace('.', '_', $genres)])['include']))));

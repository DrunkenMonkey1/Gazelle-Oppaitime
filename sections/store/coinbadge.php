...
if ($DB->has_results()) {
    [$Purchases, $Price] = $DB->next_record();
} else {
    $DB->query("
    INSERT INTO misc
           (Name,        First, Second)
    VALUES ('CoinBadge', 0,     1000)");
    [$Purchases, $Price] = [0, 1000];
}
...
<?php if (isset($_GET['confirm']) && 1 == $_GET['confirm'] && !Badges::has_badge($UserID, 255)) {
    $DB->query("
    SELECT BonusPoints
    FROM users_main
    WHERE ID = $UserID");
    [$Points] = $DB->has_results() ? $DB->next_record() : [0];
    if ($Points > $Price) {

...
if (![$Labels, $InFlow, $OutFlow] = $Cache->get_value('users_timeline')) {
    ...
    $Cache->cache_value('users_timeline', [$Labels, $InFlow, $OutFlow], mktime(0, 0, 0, date('n') + 1, 2)); //Tested: fine for Dec -> Jan
}
...
<div class="box pad center">
  <canvas class="chart" id="chart_user_classes" data-chart='{
    "type": "pie",
    "data": {
      "labels": <?php print '["' . implode('","', array_column($ClassDistribution, 'Name')) . '"]'; ?>,
      "datasets": [ {
        "data": <?php print "[" . implode(",", array_column($ClassDistribution, 'Users')) . "]"; ?>,

...
<?php
  ...
  if (($Override = check_paranoia_here('collages+'))) { ?>
        <li id="comm_collstart"<?=(2 === $Override ? ' class="paranoia_override"' : '')?>>Collages started: <?=number_format($NumCollages)?>
...
<?php
  ...
  if (($Override = check_paranoia_here('collagecontribs+'))) { ?>
        <li id="comm_collcontrib"<?=(2 === $Override ? ' class="paranoia_override"' : '')?>>Collages contributed to: <?php echo number_format($NumCollageContribs); ?>
...
<?php  } elseif ($ViewAll) { ?>
        <li>
          <span<?=(2 === $ViewCount ? ' class="paranoia_override"' : '')?>>Requests filled: <?=number_format($RequestsFilled)?></span>
...
<?php  } elseif ($ViewAll) { ?>
        <li>
          <span<?=(2 === $ViewCount ? ' class="paranoia_override"' : '')?>>Requests created: <?=number_format($RequestsCreated)?></span>
          <span<?=(2 === $ViewBounty ? ' class="paranoia_override"' : '')?>> for <?=Format::get_size($RequestsCreatedSpent)?></span>
          <a href="requests.php?type=created&amp;userid=<?=$UserID?>" class="brackets<?=(2 === $ViewAll ? ' paranoia_override' : '')?>">View</a>
        </li>
        <li>
          <span<?=(2 === $ViewCount ? ' class="paranoia_override"' : '')?>>Requests voted: <?=number_format($RequestsVoted)?></span>
...
<?php
  ...
  if ($Override = check_paranoia_here('uploads+')) { ?>
        <li id="comm_upload"<?=(2 === $Override ? ' class="paranoia_override"' : '')?>>Uploaded: <?=number_format($Uploads)?>
...
<?php
  ...
  if ($Override = check_paranoia_here('uniquegroups+')) { ?>
        <li id="comm_uniquegroup"<?=(2 === $Override ? ' class="paranoia_override"' : '')?>>Unique groups: <?=number_format($UniqueGroups)?>

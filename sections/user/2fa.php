...
if ($Type = $_POST['type'] ?? false) {
    if ('PGP' == $Type) {
        if (!empty($_POST['publickey']) && (!str_contains($_POST['publickey'], 'BEGIN PGP PUBLIC KEY BLOCK') || !str_contains($_POST['publickey'], 'END PGP PUBLIC KEY BLOCK'))) {
    ...
    if ('2FA-E' == $Type) {
        $TwoFAVerifyCode = $TwoFA->verifyCode($_POST['twofasecret'], $_POST['twofa']);
        if ($TwoFAVerifyCode) {
            $DB->query("
        UPDATE users_main
...
while ([$KeyHandle, $PublicKey, $Certificate, $Counter] = $DB->next_record()) {
...
$DB->query("
  ...
  FROM users_main
  WHERE ID = {$UserID}");
[$PublicKey, $TwoFactor] = $DB->next_record();

[$U2FRequest, $U2FSigs] = $U2F->getRegisterData($U2FRegs);
...
<?php } ?>
  ...
  <?php if (empty($PublicKey)) {
       if (!empty($TwoFactor) || count($U2FRegs) > 0) { ?>

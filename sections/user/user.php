...
if (check_perms('users_mod')) { // Person viewing is a staff member
    ...
    [
        ...
        $InfoTitle,
        $LockedAccount
    ] = $DB->next_record(MYSQLI_NUM, [8, 11]);
...
if (check_perms('site_proxy_images') && !empty($CustomTitle)) {
    $DisplayCustomTitle = preg_replace_callback(
        '#src=("?)(http.+?)(["\s>])#',
        fn ($Matches) => 'src=' . $Matches[1] . ImageTools::process($Matches[2]) . $Matches[3],
        $CustomTitle
    );
}

if (1 === $Preview) {
    $Paranoia = 0 === strlen($ParanoiaString) ? [] : $CustomParanoia;
} elseif (null !== $Paranoia) {
...
function check_paranoia_here($Setting): bool|int
{
...
<div class="thin">
  ...
  WHERE s.UserID = {$UserID}");
      ...
      WHERE u.ID = {$UserID}");
                        ...
                        $Uploads = (int) $Uploads;
                        // TODO FIX THIS SHIT ;(
                        $Level = (int) (((($Uploads ** 0.35) * 1.5) + 1) * max(((int) $Upload + ((int) $Points * 1_000_000) - (int) $Download) / (1024 ** 3),
                                1)); ?>
                        <p>This user is wild and level <?= $Level ?></p>
                        <?php if (!$OwnProfile) { ?>
                            <p>Try to capture them with <?= BONUS_POINTS ?>? The more you spend, the higher the chance
                                of capture</p>
                            <form action='store.php' method='post'>
                                <input type='hidden' name='item' value='capture_user'/>
                                <input type='hidden' name='target' value='<?= $UserID ?>'/>
                                <input type='text' name='amount' placeholder='<?= BONUS_POINTS ?>'/><input type='submit'
                                                                                                           value='Capture'/>
                            </form>
                        <?php }
                    } ?>
        ...
        <div class="box box_info box_userinfo_stats">
            ...
            <ul class="stats nobullet">
                ...
                if (($Override = check_paranoia_here('uploaded'))) {
                    ?>
                    <li class="tooltip<?= (2 === $Override ? ' paranoia_override' : '') ?>"
                        title="<?= Format::get_size($Uploaded, 5) ?>">Uploaded: <?= Format::get_size($Uploaded) ?></li>
                    <?php
                }
                if (($Override = check_paranoia_here('downloaded'))) {
                    ?>
                    <li class="tooltip<?= (2 === $Override ? ' paranoia_override' : '') ?>"
                        title="<?= Format::get_size($Downloaded, 5) ?>">
                        Downloaded: <?= Format::get_size($Downloaded) ?></li>
                    <?php
                }
                if (($Override = check_paranoia_here('ratio'))) {
                    ?>
                    <li<?= (2 === $Override ? ' class="paranoia_override"' : '') ?>>
                        Ratio: <?= Format::get_ratio_html($Uploaded, $Downloaded) ?></li>
                    <?php
                }
                if (($Override = check_paranoia_here('requiredratio')) && isset($RequiredRatio)) {
                    ?>
                    <li<?= (2 === $Override ? ' class="paranoia_override"' : '') ?>>Required Ratio:
                        <span class="tooltip"
                              title="<?= number_format((double) $RequiredRatio,
                                  5) ?>"><?= number_format((double) $RequiredRatio, 2) ?></span></li>
                    <?php
                }
                if (($Override = check_paranoia_here('downloaded'))) {
                    ?>
                    <li<?= (2 === $Override ? ' class="paranoia_override"' : '') ?>>Total Seeding: <span class="tooltip"
                ...
                if ($OwnProfile || ($Override = check_paranoia_here(false)) || check_perms('users_mod')) {
                    ?>
                    <li<?= (2 === $Override ? ' class="paranoia_override"' : '') ?>>
                        <a href="userhistory.php?action=token_history&amp;userid=<?= $UserID ?>">Tokens</a>: <?= number_format((int) $FLTokens) ?>
                    </li>
                    <?php
                }
                if (($OwnProfile || check_perms('users_mod')) && $Warned) {
                    ?>
                    <li<?= (2 === $Override ? ' class="paranoia_override"' : '') ?>>Warning expires
                        in: <?= time_diff((date('Y-m-d H:i', strtotime($Warned)))) ?></li>
                    <?php
                } ?>
            </ul>
        </div>
    ...
    WHERE r.UserID = {$UserID}");
            ...
            $RequestsVoted = $RequestsCreated = $RequestsCreatedSpent = 0;
            $TotalSpent = $RequestsCreated = $RequestsCreatedSpent = 0;
        }
    ...
    WHERE UserID = {$UserID}");
        ...
        if ($Ratio > 0) {
            $Ratio = round(Misc::DivisionByZeroFix((int) $Uploaded, (int) $Downloaded), 2);
        }
        
        $OverallRank = UserRank::overall_score($UploadedRank, $DownloadedRank, $UploadsRank, $RequestRank, $PostRank,
        ...
        <div class="box box_info box_userinfo_percentile">
            ...
            <ul class="stats nobullet">
                <?php if (($Override = check_paranoia_here('uploaded'))) { ?>
                    <li class="tooltip<?= (2 === $Override ? ' paranoia_override' : '') ?>"
                        title="<?= Format::get_size($Uploaded) ?>">Data
                        uploaded: <?= false === $UploadedRank ? 'Server busy' : number_format($UploadedRank) ?></li>
                    <?php
                }
                if (($Override = check_paranoia_here('downloaded'))) { ?>
                    <li class="tooltip<?= (2 === $Override ? ' paranoia_override' : '') ?>"
                        title="<?= Format::get_size($Downloaded) ?>">Data
                        downloaded: <?= false === $DownloadedRank ? 'Server busy' : number_format($DownloadedRank) ?></li>
                    <?php
                }
                if (($Override = check_paranoia_here('uploads+'))) { ?>
                    <li class="tooltip<?= (2 === $Override ? ' paranoia_override' : '') ?>"
                        title="<?= number_format((int) $Uploads) ?>">Torrents
                        uploaded: <?= false === $UploadsRank ? 'Server busy' : number_format($UploadsRank) ?></li>
                    <?php
                }
                if (($Override = check_paranoia_here('requestsfilled_count'))) { ?>
                    <li class="tooltip<?= (2 === $Override ? ' paranoia_override' : '') ?>"
                        title="<?= number_format((int) $RequestsFilled) ?>">Requests
                        filled: <?= false === $RequestRank ? 'Server busy' : number_format($RequestRank) ?></li>
                    <?php
                }
                if (($Override = check_paranoia_here('requestsvoted_bounty'))) { ?>
                    <li class="tooltip<?= (2 === $Override ? ' paranoia_override' : '') ?>"
                        title="<?= Format::get_size((int) $TotalSpent) ?>">Bounty
                        spent: <?= false === $BountyRank ? 'Server busy' : number_format($BountyRank) ?></li>
                <?php } ?>
                <li class="tooltip" title="<?= number_format((int) $ForumPosts) ?>">Posts
                    made: <?= false === $PostRank ? 'Server busy' : number_format($PostRank) ?></li>
                <?php if (($Override = check_paranoia_here('artistsadded'))) { ?>
                    <li class="tooltip<?= (2 === $Override ? ' paranoia_override' : '') ?>"
                        title="<?= number_format((int) $ArtistsAdded) ?>">Artists
                        added: <?= false === $ArtistsRank ? 'Server busy' : number_format($ArtistsRank) ?></li>
                    <?php
                }
                if (check_paranoia_here([
                    ...
                    <li><strong>Overall
                            rank: <?= false === $OverallRank ? 'Server busy' : number_format($OverallRank) ?></strong>
                    </li>
                <?php } ?>
            </ul>
        </div>
        <?php
        if (check_perms('users_mod', $Class) || check_perms('users_view_ips', $Class) || check_perms('users_view_keys',
                $Class)) {
            $DB->query("
      SELECT COUNT(*)
      ...
      WHERE UserID = '{$UserID}'");
            ...
            <div class="box box_info box_userinfo_history">
                ...
                <ul class="stats nobullet">
                    <?php if (check_perms('users_view_email', $Class)) { ?>
                        <li>Emails: <?= number_format($EmailChanges) ?>
                            <a href="userhistory.php?action=email2&amp;userid=<?= $UserID ?>" class="brackets">View</a>&nbsp;<a
                    ...
                    if (check_perms('users_view_ips', $Class)) {
                        ?>
                        <li>IPs: <?= number_format((int) $IPChanges) ?>
                            <a href="userhistory.php?action=ips&amp;userid=<?= $UserID ?>" class="brackets">View</a>&nbsp;<a
                        ...
                        <?php if (check_perms('users_view_ips', $Class) && check_perms('users_mod', $Class)) { ?>
                            <li>Tracker IPs: <?= number_format((int) $TrackerIPs) ?>
                                <a href="userhistory.php?action=tracker_ips&amp;userid=<?= $UserID ?>" class="brackets">View</a>
                            </li>
                            <?php
                        }
                    }
                    if (check_perms('users_view_keys', $Class)) {
                        ?>
                        <li>Passkeys: <?= number_format((int) $PasskeyChanges) ?>
                    ...
                    if (check_perms('users_mod', $Class)) {
                        ?>
                        <li>Passwords: <?= number_format((int) $PasswordChanges) ?>
                            <a href="userhistory.php?action=passwords&amp;userid=<?= $UserID ?>"
                               class="brackets">View</a></li>
                        <li>Stats: N/A <a href="userhistory.php?action=stats&amp;userid=<?= $UserID ?>"
                                          class="brackets">View</a></li>
                        <?php
                    } ?>
                </ul>
        ...
        <div class="box box_info box_userinfo_personal">
            ...
            <ul class="stats nobullet">
                ...
                ?>
                <li>Paranoia level: <span class="tooltip" title="<?= $ParanoiaLevel ?>"><?= $ParanoiaLevelText ?></span>
                </li>
                ...
                if (check_perms('users_view_invites')) {
                    if (!$InviterID) {
                        ...
                        $Invited = sprintf('<a href="user.php?id=%s">%s</a>', $InviterID, $InviterName);
                    } ?>
          ...
          WHERE InviterID = '{$UserID}'");
                        ...
                        if ($DisableInvites) {
                            ...
                            echo number_format((int) $Invites);
                        }
                        echo sprintf(' (%s)', $Pending)
                        ?></li>
                    <?php
                }
    ...
    <div class="main_column">
        ...
        if ($RatioWatchEnds && (time() < strtotime($RatioWatchEnds)) && ($Downloaded * $RequiredRatio) > $Uploaded) {
            ...
            <div class="box">
                ...
                <div class="pad">This user is currently on ratio watch and must
                    upload <?= Format::get_size(($Downloaded * $RequiredRatio) - $Uploaded) ?> in the
                    next <?= time_diff($RatioWatchEnds) ?>, or their leeching privileges will be revoked. Amount
                    downloaded while on ratio watch: <?= Format::get_size($Downloaded - $RatioWatchDownload) ?></div>
            </div>
            <?php
        }
        ?>
        <div class="box">
            <div class="head">
                <?= empty($InfoTitle) ? 'Profile' : $InfoTitle; ?>
                <span class="float_right"><a data-toggle-target="#profilediv"
                                             data-toggle-replace="Show"
      ...
      LIMIT 5");
                ...
                foreach ($RecentSnatches as $Key => $SnatchInfo) {
                    $RecentSnatches[$Key]['Artist'] = Artists::display_artists($Artists[$SnatchInfo['ID']], false,
                        true);
                }
                $Cache->cache_value(sprintf('recent_snatches_%s', $UserID), $RecentSnatches, 0); //inf cache
            }
            if (!empty($RecentSnatches)) {
                ...
                <div class="box" id="recent_snatches">
                    <div class="head">
                        Recent Snatches
                        <span class="float_right"><a onclick="$('#recent_snatches_images').gtoggle(); this.innerHTML = (this.innerHTML == 'Hide' ? 'Show' : 'Hide'); wall('#recent_snatches_images', '.collage_image', [2,3]); return false;"
        ...
        if (check_paranoia_here('uploads')) {
            $RecentUploads = $Cache->get_value(sprintf('recent_uploads_%s', $UserID));
      ...
      LIMIT 5");
                ...
                $Cache->cache_value(sprintf('recent_uploads_%s', $UserID), $RecentUploads, 0); //inf cache
            }
            if (!empty($RecentUploads)) {
                ...
                <div class="box" id="recent_uploads">
                    <div class="head">
                        Recent Uploads
                        <span class="float_right"><a onclick="$('#recent_uploads_images').gtoggle(); this.innerHTML = (this.innerHTML == 'Hide' ? 'Show' : 'Hide'); wall('#recent_uploads_images', '.collage_image', [2,3]); return false;"
                                                     class="brackets">Show</a></span>&nbsp;
                    </div>
  ...
  ORDER BY Featured DESC,
    ...
    LIMIT 5");
            ...
            <div class="box" id="collage<?= $CollageID ?>_box">
                <div class="head">
                    <?= display_str($CName) ?> - <a href="collages.php?id=<?= $CollageID ?>" class="brackets">See
                        full</a>
                    <span class="float_right">
        <a data-toggle-target="#collage<?= $CollageID ?>_box .collage_images"
           data-toggle-replace="Show"
           class="brackets">Hide</a>
      </span>
                </div>
                <div id="user_collage_images"
                     class="collage_images"
                     data-wall-child=".collage_image"
                     data-wall-size="5">
        ...
        if ((check_perms('users_view_invites')) && $Invited > 0) {
            ...
            <div class="box" id="invitetree_box">
                <div class="head">
                    Invite Tree <span class="float_right"><a data-toggle-target="#invitetree"
                                                             class="brackets">Toggle</a></span>
                </div>
        ...
        if (empty($LoggedUser['DisableRequests']) && check_paranoia_here('requestsvoted_list')) {
            ...
            if ($SphQLResult->has_results()) {
                ...
                <div class="box" id="requests_box">
                    <div class="head">
                        Requests <span class="float_right"><a data-toggle-target="#requests"
                                                              class="brackets">Show</a></span>
                    </div>
                        ...
                        <table cellpadding="6" cellspacing="1" border="0" width="100%">
                            ...
                            foreach ($SphRequests as $RequestID => $SphRequest) {
                                ...
                                if ('Music' == $CategoryName) {
                                    ...
                                    $FullName = sprintf('%s<a href="requests.php?action=view&amp;id=%s">%s [%s]</a>',
                                        $ArtistLink, $RequestID, $Request['Title'], $Request['Year']);
                                } elseif ('Audiobooks' == $CategoryName || 'Comedy' == $CategoryName) {
                                    $FullName = sprintf('<a href="requests.php?action=view&amp;id=%s">%s [%s]</a>',
                                        $RequestID, $Request['Title'], $Request['Year']);
                                } else {
                                    $FullName = sprintf('<a href="requests.php?action=view&amp;id=%s">%s</a>',
                                    ...
                                    <td>
                                        ...
                                        <div class="tags">
                                            ...
                                            foreach ($Tags as $TagID => $TagName) {
                                                $TagList[] = sprintf('<a href="requests.php?tags=%s">',
                                                        $TagName) . display_str($TagName) . '</a>';
                                            }
                ...
                <div class="box" id="staffpms_box">
                    ...
                    <table width="100%" class="message_table hidden" id="staffpms">
                        ...
                        foreach ($StaffPMs as $StaffPM) {
                            ...
                            $Resolver = $ResolverID ? Users::format_username($ResolverID, true, true, true,
                                true) : '(unresolved)'; ?>
                            <tr>
                                <td>
                                    <a href="staffpm.php?action=viewconv&amp;id=<?= $ID ?>"><?= display_str($Subject) ?></a>
                                </td>
    ...
    WHERE UserID = '{$UserID}'");
            ...
            if ($DB->has_results()) {
                ...
                <div class="box">
                    ...
                    <div class="pad">
                        <div id="forumwarningslinks"
                             class="AdminComment"
                             style="width: 98%;"><?= Text::full_format($ForumWarnings) ?></div>
                    </div>
                </div>
                <?php
            }
            ...
            <form class="manage_form" name="user" id="form" action="user.php" method="post">
                ...
                <div class="box box2" id="staff_notes_box">
                    <div class="head">
                        Staff Notes
                        <a href="#"
                           name="admincommentbutton"
                           onclick="ChangeTo('text'); return false;"
        ...
        <a data-toggle-target="#staffnotes" class="brackets">Toggle</a>
                    ...
                    <div id="staffnotes" class="pad">
                        ...
                        <div id="admincommentlinks"
                             class="AdminComment"
                             style="width: 98%;"><?= Text::full_format($AdminComment) ?></div>
                        <textarea id="admincomment"
                                  onkeyup="resize('admincomment');"
                                  class="AdminComment hidden"
                                  name="AdminComment"
                                  cols="65"
                                  rows="26"
                ...
                <table class="layout box" id="user_info_box">
                    ...
                    <?php if (check_perms('users_edit_usernames', $Class)) { ?>
                        <tr>
                            ...
                            <td><input type="text" size="20" name="Username" value="<?= display_str($Username) ?>"/>
                            </td>
                        </tr>
                        <?php
                    }
                            ...
                            <td>
                                <select name="Class">
                                    ...
                                    foreach ($ClassLevels as $CurClass) {
                                        if (check_perms('users_promote_below',
                                                $Class) && $CurClass['ID'] >= $LoggedUser['EffectiveClass']) {
                                            break;
                                        }
                                        ...
                                        $Selected = $Class == $CurClass['Level'] ? ' selected="selected"' : ''; ?>
                    ...
                    if (check_perms('users_give_donor')) {
                        ...
                        <tr>
                            ...
                            <td><input type="checkbox"
                                       name="Donor"<?php if (1 == $Donor) { ?> checked="checked"<?php } ?> /></td>
                        </tr>
                        <?php
                    }
      ...
      ORDER BY p.Name");
                                ...
                                while ([$PermID, $PermName, $IsSet] = $DB->next_record()) {
                                    ...
                                    <input type="checkbox"
                                           ...
                                           value="<?= $PermID ?>"<?php if ($IsSet) { ?> checked="checked"<?php } ?> />&nbsp;
                    ...
                    if (check_perms('users_make_invisible')) {
                        ...
                        <tr>
                            ...
                            <td><input type="checkbox"
                                       name="Visible"<?php if (1 == $Visible) { ?> checked="checked"<?php } ?> /></td>
                        </tr>
                        <?php
                    }
                    
                    if (check_perms('users_edit_ratio',
                            $Class) || (check_perms('users_edit_own_ratio') && $UserID == $LoggedUser['ID'])) {
                        ?>
                        <tr>
                            <td class="label tooltip"
                                title="Upload amount in bytes. Also accepts e.g. +20GB or -35.6364MB on the end.">
                                Uploaded:
                            </td>
                            <td>
                        ...
                        <tr>
                            <td class="label tooltip"
                                title="Download amount in bytes. Also accepts e.g. +20GB or -35.6364MB on the end.">
                                Downloaded:
                            </td>
                            <td>
    ...
    GROUP BY um.ID");
                                    if ($DB->has_results()) {
                                        ...
                                        $PointsRate = (0.5 + (0.55 * ($NumTorr * (sqrt(($TSize / $NumTorr) / 1_073_741_824) * pow(1.5,
                                                                ($TTime / $NumTorr) / (24 * 365))))) / (max(1,
                                                    sqrt(($TSeeds / $NumTorr) + 4) / 3))) ** 0.95;
                                    }
                                    $PointsRate = (int) max(min($PointsRate, ($PointsRate * 2) - ($BonusPoints / 1440)),
                                        0);
                    ...
                    if (check_perms('admin_manage_fls') || (check_perms('users_mod') && $OwnProfile)) {
                        ...
                        <tr>
                            <td class="label tooltip"
                                title="This is the message shown in the right-hand column on /staff.php">FLS/Staff
                                remark:
                            </td>
                            <td><input type="text"
                                       class="wide_input_text"
                    ...
                    if (check_perms('users_edit_reset_keys')) {
                        ...
                        <tr>
                            ...
                            <td>
                                <input type="checkbox" name="ResetRatioWatch" id="ResetRatioWatch"/>
                                <label for="ResetRatioWatch">Ratio watch</label> |
                                <input type="checkbox" name="ResetPasskey" id="ResetPasskey"/>
                                <label for="ResetPasskey">Passkey</label> |
                                <input type="checkbox" name="ResetAuthkey" id="ResetAuthkey"/>
                                <label for="ResetAuthkey">Authkey</label> |
                                <input type="checkbox" name="ResetIPHistory" id="ResetIPHistory"/>
                                <label for="ResetIPHistory">IP history</label> |
                                <input type="checkbox" name="ResetEmailHistory" id="ResetEmailHistory"/>
                                <label for="ResetEmailHistory">Email history</label>
                                <br/>
                                <input type="checkbox" name="ResetSnatchList" id="ResetSnatchList"/>
                                <label for="ResetSnatchList">Snatch list</label> |
                                <input type="checkbox" name="ResetDownloadList" id="ResetDownloadList"/>
                                <label for="ResetDownloadList">Download list</label>
                            </td>
                        </tr>
                        <?php
                    }
                    ...
                    if (check_perms('users_edit_badges')) {
                        ...
                        <tr id="user_badge_edit_tr">
                            ...
                            <td>
                                ...
                                $UserBadgeIDs = [];
                                $UserBadgeIDs = array_keys(Badges::get_badges($UserID));
                                $i = 0;
                                foreach (array_keys($AllBadges) as $BadgeID) {
                                    ?><input type="checkbox"
                                             name="badges[]"
                                             class="badge_checkbox"
                                             value="<?= $BadgeID ?>" <?= (in_array($BadgeID, $UserBadgeIDs,
                                    true)) ? " checked" : "" ?>/><?= Badges::display_badge($BadgeID, true) ?>
                                    <?php ++$i;
                                    if (0 == $i % 8) {
                                        echo "<br />";
                                    }
                ...
                <table class="layout box" id="warn_user_box">
                    ...
                    <?php } ?>
                    <tr>
                        <td class="label tooltip" title="This message *will* be sent to the user in the warning PM!">
                            Warning reason:
                        </td>
                        <td>
                    ...
                    <table class="layout box">
                        ...
                        <tr>
                            ...
                            <td>
                                <input type="checkbox"
                                       name="LockAccount"
                                       id="LockAccount" <?php if ($LockedAccount) { ?> checked="checked" <?php } ?>/>
                            </td>
                        </tr>
                        <tr>
                            ...
                            <td>
                                <select name="LockReason">
                                    <option value="---">---</option>
                                    <option value="<?= STAFF_LOCKED ?>" <?php if (STAFF_LOCKED == $LockedAccount) { ?> selected <?php } ?>>
                                        Staff Lock
                                    </option>
                                </select>
                            </td>
      ...
      ORDER BY Time ASC");
                        ...
                        <input type="checkbox"
                               name="DisablePosting"
                               id="DisablePosting"<?php if (1 == $DisablePosting) { ?> checked="checked"<?php } ?> />
                        <label for="DisablePosting">Posting</label>
                        <?php if (check_perms('users_disable_any')) { ?> |
                            <input type="checkbox"
                                   name="DisableAvatar"
                                   id="DisableAvatar"<?php if (1 == $DisableAvatar) { ?> checked="checked"<?php } ?> />
                            <label for="DisableAvatar">Avatar</label> |
                            <input type="checkbox"
                                   name="DisableForums"
                                   id="DisableForums"<?php if (1 == $DisableForums) { ?> checked="checked"<?php } ?> />
                            <label for="DisableForums">Forums</label> |
                            <input type="checkbox"
                                   name="DisableIRC"
                                   id="DisableIRC"<?php if (1 == $DisableIRC) { ?> checked="checked"<?php } ?> /> <label
                                    for="DisableIRC">IRC</label> |
                            <input type="checkbox"
                                   name="DisablePM"
                                   id="DisablePM"<?php if (1 == $DisablePM) { ?> checked="checked"<?php } ?> /> <label
                                    for="DisablePM">PM</label> |
                            <br/><br/>

                            <input type="checkbox"
                                   name="DisableLeech"
                                   id="DisableLeech"<?php if (0 == $DisableLeech) { ?> checked="checked"<?php } ?> />
                            <label for="DisableLeech">Leech</label> |
                            <input type="checkbox"
                                   name="DisableRequests"
                                   id="DisableRequests"<?php if (1 == $DisableRequests) { ?> checked="checked"<?php } ?> />
                            <label for="DisableRequests">Requests</label> |
                            <input type="checkbox"
                                   name="DisableUpload"
                                   id="DisableUpload"<?php if (1 == $DisableUpload) { ?> checked="checked"<?php } ?> />
                            <label for="DisableUpload">Torrent upload</label> |
                            <input type="checkbox"
                                   name="DisablePoints"
                                   id="DisablePoints"<?php if (1 == $DisablePoints) { ?> checked="checked"<?php } ?> />
                            <label for="DisablePoints"><?= BONUS_POINTS ?></label>
                            <br/><br/>

                            <input type="checkbox"
                                   name="DisableTagging"
                                   id="DisableTagging"<?php if (1 == $DisableTagging) { ?> checked="checked"<?php } ?> />
                            <label for="DisableTagging"
                                   class="tooltip"
                                   title="This only disables a user's ability to delete tags.">Tagging</label> |
                            <input type="checkbox"
                                   name="DisableWiki"
                                   id="DisableWiki"<?php if (1 == $DisableWiki) { ?> checked="checked"<?php } ?> />
                            <label for="DisableWiki">Wiki</label> |
                            <input type="checkbox"
                                   name="DisablePromotion"
                                   id="DisablePromotion"<?php if (1 == $DisablePromotion) { ?> checked="checked"<?php } ?> />
                            <label for="DisablePromotion">Promotions</label> |
                            <input type="checkbox"
                                   name="DisableInvites"
                                   id="DisableInvites"<?php if (1 == $DisableInvites) { ?> checked="checked"<?php } ?> />
                            <label for="DisableInvites">Invites</label>
                            </td>
                            </tr>
                            <tr>
                                ...
                                <td>
                                    ...
                                    <label for="SendHackedMail">Send hacked account email</label> to
                                    <select name="HackedEmail">
                                        ...
                                        foreach ($Emails as $Email) {
                                            ...
                                            <option value="<?= display_str($Address) ?>"><?= display_str($Address) ?>
                                                - <?= display_str($IP) ?></option>
                                            <?php
                                        } ?>
                                    </select>
                                </td>
                    ...
                    if (check_perms('users_disable_any')) {
                        ...
                        <tr>
                            ...
                            <td>
                                <select name="UserStatus">
                                    <option value="0"<?php if ('0' == $Enabled) { ?> selected="selected"<?php } ?>>
                                        Unconfirmed
                                    </option>
                                    <option value="1"<?php if ('1' == $Enabled) { ?> selected="selected"<?php } ?>>
                                        Enabled
                                    </option>
                                    <option value="2"<?php if ('2' == $Enabled) { ?> selected="selected"<?php } ?>>
                                        Disabled
                                    </option>
                        ...
                        <tr>
                            <td class="label tooltip" title="Enter a comma-delimited list of forum IDs.">Restricted
                                forums:
                            </td>
                            <td>
                                <input type="text"
                                       class="wide_input_text"
                                       name="RestrictedForums"
                                       value="<?= display_str($RestrictedForums) ?>"/>
                            </td>
                        </tr>
                        <tr>
                            <td class="label tooltip" title="Enter a comma-delimited list of forum IDs.">Extra forums:
                            </td>
                            <td>
                                <input type="text"
                                       class="wide_input_text"
                                       name="PermittedForums"
                ...
                <table class="layout box" id="submit_box">
                    ...
                    <tr>
                        <td class="label tooltip" title="This message will be entered into staff notes only.">Reason:
                        </td>
                        <td>
                            <textarea rows="1"
                                      cols="35"
                                      ...
                                      name="Reason"
                                      id="Reason"
                                      onkeyup="resize('Reason');"></textarea>
                        </td>
                    </tr>
                    <tr>

...
foreach ($Badges as $BadgeID => $OldDisplayed) {
    if (in_array($BadgeID, $BadgeIDs, true)) { // Is the current badge in the list of badges the user wants to display?
        $Displayed = true;
        $DisplayedBadgeIDs[] = $BadgeID;
        if (0 == $OldDisplayed) { // The user wants to display a badge that wasn't displayed before
            $BadgesChanged = true;
        }

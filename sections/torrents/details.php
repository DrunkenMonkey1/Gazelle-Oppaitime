...
$RevisionID = !empty($_GET['revisionid']) && is_number($_GET['revisionid']) ? $_GET['revisionid'] : 0;
...
<div class="thin">
    <div class="header">
        ...
        <div class="linkbox">
            ...
            <?php } ?>
            <a href="#"
               id="subscribelink_torrents<?= $GroupID ?>"
               class="brackets"
               onclick="SubscribeComments('torrents', <?= $GroupID ?>); return false;"><?= false !== Subscriptions::has_subscribed_comments('torrents',
                    $GroupID) ? 'Unsubscribe' : 'Subscribe' ?></a>
            <?php
            if ('Movies' == $Categories[$GroupCategoryID - 1] || 'Anime' == $Categories[$GroupCategoryID - 1] || 'Manga' == $Categories[$GroupCategoryID - 1] || 'Games' == $Categories[$GroupCategoryID - 1]) { ?>
            ...
            <div class="head">
                ...
                if (count($CoverArt) > 0) {
                    if (empty($LoggedUser['ShowExtraCovers'])) {
                        foreach (array_keys($CoverArt) as $Index) {
                            ?>
                            <span id="cover_controls_
                  <?= ($Index) ?>
                  "
...
<?php } elseif (0 == $Index && count($CoverArt) > 0) { ?>
...
?>
          ...
          <a class="brackets show_all_covers" href="#">Hide</a>
            ...
            <div id="covers">
                <div id="cover_div_<?= $Index ?>">
                    ...
                    <?php } else { ?>
                        <div><img width="100%"
                                  src="<?= STATIC_SERVER ?>common/noartwork/nocover.png"
                ...
                <?php foreach ($CoverArt as $Cover) {
                    ...
                    <div id="cover_div_<?= $Index ?>"<?= (empty($LoggedUser['ShowExtraCovers']) ? ' style="display: none;"' : '') ?>>
                        <div>
                            <?php
                            if (empty($LoggedUser['ShowExtraCovers'])) {
                                $Src = 'src="" data-gazelle-temp-src="' . ImageTools::process($Image,
                                        'thumb') . '" lightbox-img="' . ImageTools::process($Image) . '"';
                            } else {
                                $Src = 'src="' . ImageTools::process($Image,
                                        'thumb') . '" lightbox-img="' . ImageTools::process($Image) . '"';
                            } ?>
                            <img id="cover_<?= $Index ?>"
                                 class="lightbox-init"
                        ...
                        <ul class="stats nobullet">
                            <li>
                                <?= $Summary ?>
                                <?= (check_perms('users_mod') ? ' added by ' . Users::format_username($AddedBy, false,
            ...
            <div class="box box_addartists">
                ...
                <div class="body">
                    <form class="flex_input_container" name="artists" action="torrents.php" method="post">
                        ...
                        <input type="text"
                               id="artist"
                               name="artistname[]" <?php Users::has_autocomplete_enabled('other'); ?> />
        ...
        <div class="box box_tags">
            <div class="head">
                ...
                if (!empty($DeletedTag)) { ?>
                    ...
                    <a class="brackets" href="#" onclick="$('#undo_tag_delete_form').raw().submit(); return false;">Undo
                        delete</a>
                            ...
                            <div class="edit_tags_votes float_right">
                                <?php if (check_perms('users_warn')) { ?>
                                    <a href="user.php?id=<?= $Tag['userid'] ?>"
                                       title="View the profile of the user that added this tag"
                                       class="brackets tooltip view_tag_user">U</a>
                                <?php } ?>
                                <?php if (empty($LoggedUser['DisableTagging']) && check_perms('site_delete_tag')) { ?>
                                    <span class="remove remove_tag"><a href="torrents.php?action=delete_tag&amp;groupid=<?= $GroupID ?>&amp;tagid=<?= $Tag['id'] ?>&amp;auth=<?= $LoggedUser['AuthKey'] ?>"
                                                                       class="brackets tooltip"
                                                                       title="Remove tag">X</a></span>
                                <?php } ?>
                            </div>
                        </li>
                        <?php
            ...
            <div class="box box_addtag">
                ...
                <div class="body">
                    <form class="flex_input_container" name="tags" action="torrents.php" method="post">
                        ...
                        <input type="text"
                               name="tagname"
                               id="tagname" <?php Users::has_autocomplete_enabled('other'); ?> />
                        <input type="submit" value="Add"/>
                    </form>
                    <br/>
    ...
    <div class="main_column">
        <div class="box">
            <table class="torrent_table details<?= $GroupFlags['IsSnatched'] ? ' snatched' : '' ?>"
                   id="torrent_details">
    ...
    <table class="reportinfo_table">
      <tr class="colhead_dark" style="font-weight: bold;">
        <td>This torrent has ' . $NumReports . ' active ' . (1 === $NumReports ? 'report' : 'reports') . ":</td>
      </tr>";
                        ...
                        foreach ($Reports as $Report) {
                            ...
                            $ReportInfo .= "
      <tr>
        <td>$ReportLinks " . time_diff($Report['ReportedTime'], 2, true,
                                    true) . ' for the reason "' . $ReportType['title'] . '":
        ...
        <strong>Size</strong>
                    ...
                    if ('}}}' == substr($FileList, -3)) { // Old style
                        ...
                        foreach ($FileListSplit as $File) {
                            ...
                            $FileTable .= sprintf("\n<tr class=\"row\"><td>%s</td><td class=\"number_column nobr\">%s</td></tr>",
                    ...
                    if ($Language) {
                        if ($Subber && (3 == $GroupCategoryID || 4 == $GroupCategoryID)) {
                    ...
                    if ($Subbing) {
                        if ($Subber) {
                            if ((2 == $GroupCategoryID || 1 == $GroupCategoryID) && "RAW" != $Subbing) {
                    ...
                    if ($Archive) {
                        $ExtraInfo .= $AddExtra . display_str('Archived (' . $Archive . ')');
                        $AddExtra = ' / ';
                    }
                    if (!$ExtraInfo || 5 == $GroupCategoryID) {
                        $ExtraInfo = $GroupName;
                        $AddExtra = ' / ';
                    }
                    if ($IsLeeching) {
      ...
      WHERE TorrentID = $TorrentID");
                        if ($DB->has_results()) {
                            ...
                            $ExtraInfo .= " <strong>(" . str_replace([
                                    ...
                                    time_diff(max($ExpiryTime, time()), 1, false)) . ")</strong>";
                        }
                    }
                    if ($PersonalFL) {
                    ...
                    if (!$Censored) {
                        $ExtraInfo .= $AddExtra . Format::torrent_label('Uncensored');
                        $AddExtra = ' / ';
                    }
                    if (!empty($BadTags)) {
                    ...
                    if (!empty(G::$LoggedUser) && (G::$LoggedUser['ShowMagnets'] ?? false)) {
                        ...
                        $TorrentMG = "magnet:?dn=" . rawurlencode($TorrentFileName) . "&xt=urn:btih:" . $InfoHash . "&as=https://" . SITE_DOMAIN . "/" . str_replace('&amp;',
                                '%26',
                                $TorrentDL) . "&tr=" . implode("/" . $LoggedUser['torrent_pass'] . "/announce&tr=",
                                ANNOUNCE_URLS[0]) . "/" . $LoggedUser['torrent_pass'] . "/announce&xl=" . $Size;
                    } ?>

                    <tr class="torrent_row groupid_<?= $GroupID ?> group_torrent<?= ($IsSnatched ? ' snatched_torrent' : '') ?>"
...
<?php } ?>
            | <a href="torrents.php?torrentid=<?= $TorrentID ?>" class="tooltip" title="Permalink">PL</a>
                    ...
                    <tr class=" groupid_<?= $GroupID ?> torrentdetails pad <?php if (!isset($_GET['torrentid']) || $_GET['torrentid'] != $TorrentID) { ?>hidden<?php } ?>"
                        ...
                        <td colspan="5">
                            <div id="release_<?= $TorrentID ?>" class="no_overflow">
                                <blockquote>
                                    ...
                                    <?php if (0 == $Seeders) {
                                        if ($LastActive && time() - strtotime($LastActive) >= 1_209_600) { ?>
                                    ...
                                    if ((0 == $Seeders && $LastActive && time() - strtotime($LastActive) >= 345678 && time() - strtotime($LastReseedRequest) >= 864000) || check_perms('users_mod')) { ?>
                                        <br/>
                                        <a href="torrents.php?action=reseed&amp;torrentid=<?= $TorrentID ?>&amp;groupid=<?= $GroupID ?>"
                            ...
                            <div class="linkbox">
                                <a href="#"
                                   class="brackets"
                                   onclick="show_peers('<?= $TorrentID ?>', 0); return false;">View peer list</a>
                                <?php if (check_perms('site_view_torrent_snatchlist')) { ?>
                                    <a href="#"
                                       class="brackets tooltip"
                                       onclick="show_downloads('<?= $TorrentID ?>', 0); return false;"
                                       title="View the list of users that have clicked the &quot;DL&quot; button.">View
                                        download list</a>
                                    <a href="#"
                                       class="brackets tooltip"
                                       onclick="show_snatches('<?= $TorrentID ?>', 0); return false;"
                                       title="View the list of users that have reported a snatch to the tracker.">View
                                        snatch list</a>
                                <?php } ?>
                                <a href="#" class="brackets" onclick="show_files('<?= $TorrentID ?>'); return false;">View
                                    file list</a>
                                <?php if ($Reported) { ?>
                                    <a href="#"
                                       class="brackets"
                                       onclick="show_reported('<?= $TorrentID ?>'); return false;">View report
                                        information</a>
                                <?php } ?>
                            </div>
                            ...
                            if (!empty($MediaInfo)) {
                                ...
                                if (!empty($parsed)) {
                                    ...
                                    <div class="spoilerContainer hideContainer">
                                        <input type="button" class="spoilerButton" value="Show MediaInfo"/>
                ...
                <table id="requests" class="request_table hidden">
                    ...
                    <?php foreach ($Requests as $Request) {
                        ...
                        <tr class="requestrows row">
                            <td><a href="requests.php?action=view&amp;id=<?= $Request['ID'] ?>"><?= $RequestDesc ?></a>
                            </td>
                            <td>
                                <span id="vote_count_<?= $Request['ID'] ?>"><?= count($RequestVotes['Voters']) ?></span>
                                <?php if (check_perms('site_vote')) { ?>
                                    &nbsp;&nbsp; <a href="javascript:Vote(0, <?= $Request['ID'] ?>)"
                                                    class="brackets">+</a>
                                <?php } ?>
                            </td>
      ...
      AND CategoryID != '0'");
        ...
        if (count($Collages) > 0) {
            ...
            <table class="box collage_table" id="collages">
                <tr class="colhead">
                    <td width="85%"><a href="#">&uarr;</a>&nbsp;This content is
                        in <?= number_format(count($Collages)) ?>
                        collection<?= ((count($Collages) > 1) ? 's' : '') ?><?= $SeeAll ?></td>
                    <td># torrents</td>
                </tr>
    ...
    WHERE ct.GroupID = '$GroupID'
      ...
      AND CategoryID = '0'");
        ...
        if (count($PersonalCollages) > 0) {
            ...
            <table class="box collage_table" id="personal_collages">
                <tr class="colhead">
                    <td width="85%"><a href="#">&uarr;</a>&nbsp;This content is
                        in <?= number_format(count($PersonalCollages)) ?> personal
                        collection<?= ((count($PersonalCollages) > 1) ? 's' : '') ?><?= $SeeAll ?></td>
                    <td># torrents</td>
                </tr>
                <?php foreach ($Indices as $i) {
        ...
        <div class="box torrent_description">
            <div class="head">
                <a href="#">&uarr;</a>&nbsp;<strong><?= (empty($ReleaseType) ? 'Info' : $ReleaseTypes[$ReleaseType] . ' info') ?></strong>
            </div>
        ...
        <div class="box torrent_screenshots_box <?php if (count($Screenshots) === 0) {
            echo 'dead';
        } ?>">
            <div class="head"><a href="#">&uarr;</a>&nbsp;<strong>
                    <?= (3 == $GroupCategoryID) ? 'Samples' : 'Screenshots' ?> (<?= count($Screenshots) ?>)</strong>
                <?php
                if (count($Screenshots) > 0) {
                    ?>
                    <a class="float_right brackets"
                       data-toggle-target=".torrent_screenshots"
                       data-toggle-replace="Hide">Show</a>
                    <?php
                }
      ...
      WHERE GroupID = $GroupID");
                ...
                if (in_array($LoggedUser['ID'], $DBCollect,
                        true) || check_perms('torrents_edit') || check_perms('screenshots_add') || check_perms('screenshots_delete')) {
                    ?>
                    <a class="brackets"
                ...
                foreach ($Screenshots as $Screenshot) {
                    ...
                    if (check_perms('users_mod')) {
                        ?><img class='tooltip lightbox-init'
                               title='<?= Users::format_username($Screenshot['UserID'], false, false,
                                   false) ?> - <?= time_diff($Screenshot['Time']) ?>'
                               lightbox-img="<?= $SSURL ?>"
                               src="<?= $ThumbURL ?>" /><?php
                    } else {
                        ?><img class='tooltip lightbox-init'

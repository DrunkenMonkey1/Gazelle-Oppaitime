...
function get_group_info($GroupID, $Return = true, $RevisionID = 0, $PersonalProperties = true, $ApiCall = false)
{
    global $Cache, $DB;
    if (!$RevisionID) {
        $TorrentCache = $Cache->get_value(sprintf('torrents_details_%s', $GroupID));
    }
    if ($RevisionID || !is_array($TorrentCache)) {
      ...
      ORDER BY
        ...
        if (0 === count($TorrentList) && false == $ApiCall) {
            header('Location: log.php?search=' . (empty($_GET['torrentid']) ? sprintf('Group+%s',
                    $GroupID) : sprintf('Torrent+%s', $_GET['torrentid'])));
            die();
        }
    
        if (0 === count($TorrentList) && true == $ApiCall) {
            return null;
        }
        $DBCollect = $DB->collect('Seeders');
        $CacheTime = in_array(0, $DBCollect, true) ? 600 : 3600;
        // Store it all in cache
        if (!$RevisionID) {
            $Cache->cache_value(sprintf('torrents_details_%s', $GroupID), [$TorrentDetails, $TorrentList], $CacheTime);
        }
    ...
    if ($PersonalProperties) {
        ...
        $TorrentDetails['Flags'] = ['IsSnatched' => false, 'IsLeeching' => false, 'IsSeeding' => false];
...
function set_torrent_logscore($TorrentID): void
{
    global $DB;
    $DB->query("
    UPDATE torrents
...
function get_group_requests($GroupID)
{
    ...
    $Requests = $Cache->get_value(sprintf('requests_group_%s', $GroupID));
    if (false === $Requests) {
        $DB->query("
      SELECT ID
      FROM requests
      WHERE GroupID = {$GroupID}
        ...
        $Cache->cache_value(sprintf('requests_group_%s', $GroupID), $Requests, 0);
    }
        ...
        <td>This torrent has ' . $NumReports . ' active ' . (1 === $NumReports ? 'report' : 'reports') . ":</td>
            ...
            foreach ($Reports as $Report) {
                if (check_perms('admin_reports')) {
                    ...
                    $ReportLinks = sprintf('<a href="user.php?id=%s">%s</a> <a href="reportsv2.php?view=report&amp;id=%s">reported it</a>',
                        $ReporterID, $ReporterName, $Report[ID]);
                } else {
  ...
  <table class="filelist_table">
    <tr class="colhead_dark">
      <td>
        ...
        <div class="filelist_path float_right">' . ($FilePath ? sprintf('/%s/', $FilePath) : '') . '</div>
      </td>
      <td>
        <strong>Size</strong>
      </td>
        ...
        if ('}}}' == substr($FileList, -3)) { // Old style
            ...
            foreach ($FileListSplit as $File) {
                ...
                $FileTable .= sprintf("\n<tr class=\"row\"><td>%s</td><td class=\"number_column\">%s</td></tr>", $Name,
                    Format::get_size($FileSize));
            }
        } else {
            $FileListSplit = explode("\n", $FileList);
            foreach ($FileListSplit as $File) {
                $FileInfo = Torrents::filelist_get_file($File);
                $FileTable .= sprintf("\n<tr class=\"row\"><td>%s</td><td class=\"number_column\">%s</td></tr>",
...
<?php } ?>
        ...
        <tr class="<?= (isset($ReleaseType) ? 'releases_' . $ReleaseType : '') ?> groupid_<?= ($GroupID) ?> edition_<?= ($EditionID) ?> torrentdetails pad<?php if (!isset($_GET['torrentid']) || $_GET['torrentid'] != $TorrentID) { ?> hidden<?php } ?>"
            ...
            <td colspan="5">
                ...
                <?php } ?>
                <div class="linkbox">
                    <a href="#" class="brackets" onclick="show_peers('<?= ($TorrentID) ?>', 0); return false;">View peer
                        list</a>
                    <?php if (check_perms('site_view_torrent_snatchlist')) { ?>
                        <a href="#"
                           class="brackets tooltip"
                           onclick="show_downloads('<?= ($TorrentID) ?>', 0); return false;"
                           title="View the list of users that have clicked the &quot;DL&quot; button.">View download
                            list</a>
                        <a href="#"
                           class="brackets tooltip"
                           onclick="show_snatches('<?= ($TorrentID) ?>', 0); return false;"
                           title="View the list of users that have reported a snatch to the tracker.">View snatch
                            list</a>
                    <?php } ?>
                    <a href="#" class="brackets" onclick="show_files('<?= ($TorrentID) ?>'); return false;">View file
                        list</a>
                    <?php if ($Reported) { ?>
                        <a href="#" class="brackets" onclick="show_reported('<?= ($TorrentID) ?>'); return false;">View

...
$CacheGet_value = $Cache->get_value(sprintf('torrent_%s_lock', $TorrentID));
...
[$UploaderID, $GroupID, $Size, $InfoHash, $Name, $ArtistName, $Time, $Snatches] = $DB->next_record(MYSQLI_NUM, false);
...
Misc::write_log(sprintf('Torrent %s (%s) (', $TorrentID, $Name) . number_format($Size / (1024 * 1024), 2) . ' MB) (' . strtoupper($InfoHash[1]) . ') was deleted by ' . $LoggedUser['Username'] . ': ' . $_POST['reason'] . ' ' . $_POST['extra']);

    ...
    <div class="box box2 thin">
      <div class="head">
        ...
        <span class="float_right">
          <a data-toggle-target="#postform" data-toggle-replace="<?=(('editblog' != $_REQUEST['action']) ? 'Hide' : 'Show')?>" class="brackets"><?=(('editblog' != $_REQUEST['action']) ? 'Show' : 'Hide')?></a>
        </span>
      </div>
      <form class="<?=((empty($_GET['action'])) ? 'create_form' : 'edit_form')?>" name="blog_post" action="staffblog.php" method="post">
        <div id="postform" class="pad<?=('editblog' != $_REQUEST['action']) ? ' hidden' : '' ?>">
          <input type="hidden" name="action" value="<?=((empty($_GET['action'])) ? 'takenewblog' : 'takeeditblog')?>" />
          <input type="hidden" name="auth" value="<?=$LoggedUser['AuthKey']?>" />
<?php    if (!empty($_GET['action']) && 'editblog' == $_GET['action']) { ?>
...
<?php    } ?>
        ...
        echo ' value="' . display_str($Title) . '"';
          ...
          <div class="field_div">
            <h3>Body</h3>
            <textarea name="body" cols="95" rows="15"><?php if (!empty($Body)) {
...
foreach ($Blog as $BlogItem) {
    [$BlogID, $Author, $Title, $Body, $BlogTime] = $BlogItem;

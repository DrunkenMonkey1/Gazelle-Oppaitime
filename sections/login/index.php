    ...
    if (!empty($_REQUEST['key'])) {
      ...
      WHERE i.ResetKey = ?
        ...
        if ($UserID && strtotime($Expires) > time()) {
            ...
            $Validate->SetFields('password', '1', 'regex', 'You entered an invalid password. Any password at least 6 characters long is accepted, but a strong password is 8 characters or longer, contains at least 1 lowercase and uppercase letter, contains at least a number or symbol', ['regex' => '/(?=^.{6,}$).*$/']);
...
// Normal login
    ...
    $Validate->SetFields('password', '1', 'string', 'You entered an invalid password.', ['minlength' => '6', 'maxlength' => '307200']);
    ...
    function log_attempt()
    {
        ...
        $Cache->cache_value('login_attempts_' . db_string($_SERVER['REMOTE_ADDR']), [$Attempts, ($Attempts > 5)], 60*60*$Attempts);
    ...
    if (isset($_POST['username']) && !empty($_POST['username']) && isset($_POST['password']) && !empty($_POST['password'])) {
        ...
        WHERE Username = ?
          AND Username != ''", $_POST['username']);
            [$UserID, $PermissionID, $CustomPermissions, $PassHash, $TwoFactor, $Enabled] = $DB->next_record(MYSQLI_NUM, [2]);
            if (!$Banned) {
              ...
              // Check if the current login attempt is from a location previously logged in from
                  ...
                  WHERE UserID = ?", $UserID);
                                ...
                                if (count($IPs) > 0) { // Always allow first login
                                    ...
                                    if (!in_array($CurrentASN, $PastASNs, true) && FEATURE_ENFORCE_LOCATIONS) {
                                        ...
                                        if (true !== $Cache->get_value('new_location_' . $UserID . '_' . $CurrentASN)) {
                                            $DB->query("
                        SELECT
                          UserName,
                          Email
                ...
                WHERE UserID = ?", $UserID);
                            ...
                            if (0 == sizeof($U2FRegs) || !isset($U2FErr)) {

    ...
    <div class="box pad">
        ...
        <?php } else { ?>
            <form class="search_form"
                  name="<?= ('sentbox' === $Section ? 'sentbox' : 'inbox') ?>"
                  action="inbox.php"
                  method="get"
                  id="searchbox">
                <div>
                    ...
                    <input type="radio"
                           name="searchtype"
                           value="user"<?= (empty($_GET['searchtype']) || 'user' === $_GET['searchtype'] ? ' checked="checked"' : '') ?> />
                    User
                    <input type="radio"
                           name="searchtype"
                           value="subject"<?= (!empty($_GET['searchtype']) && 'subject' === $_GET['searchtype'] ? ' checked="checked"' : '') ?> />
                    Subject
                    <input type="radio"
                           name="searchtype"
                           value="message"<?= (!empty($_GET['searchtype']) && 'message' === $_GET['searchtype'] ? ' checked="checked"' : '') ?> />
                    Message
...
<?php } ?>
                    ...
                    <input type="search"
                           name="search"
                           placeholder="<?= (empty($_GET['search']) ? 'Search ' . ('sentbox' === $Section ? 'sentbox' : 'inbox') : display_str($_GET['search'])) ?>"/>
                </div>
            </form>
                ...
                <table class="message_table checkboxes">
                    ...
                    <?php } else {
                        while ([
                            ...
                            <tr class="<?= $RowClass ?>">
                                <td class="center"><input type="checkbox" name="messages[]=" value="<?= $ConvID ?>"/>
                                </td>
                                <td>
                                    <?php
                                ...
                                <?php if (check_perms('users_mod')) { ?>
                                    <td><?= (($ForwardedID && $ForwardedID != $LoggedUser['ID']) ? Users::format_username($ForwardedID,
    ...
    <div class="linkbox">
        <?php echo sprintf('		%s%s', $Pages, PHP_EOL); ?>
    </div>
</div>
<?php
View::show_footer();

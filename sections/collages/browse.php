...
if (!empty($_GET['type'])) {
    $Type = $_GET['type'];
    if (!in_array($Type, ['c.name', 'description'], true)) {
        $Type = 'c.name';
    }
} else {
    $Type = 'c.name';
...
if (isset($_GET['action']) && 'mine' === $_GET['action']) {
    $SQL = $BaseSQL;
    $SQL .= "
...
<?php    } ?>
        ...
        <tr id="search_name_description">
          ...
          <td>
            <input type="radio" name="type" value="c.name" <?php if ('c.name' === $Type) {
    echo 'checked="checked" ';
} ?>/> Names&nbsp;&nbsp;
            <input type="radio" name="type" value="description" <?php if ('description' === $Type) {
    echo 'checked="checked" ';
} ?>/> Descriptions
          </td>
        </tr>
...
<?php    foreach ($OrderVals as $Cur) { ?>
              <option value="<?=$Cur?>"<?php if (isset($_GET['order_by']) && $_GET['order_by'] === $Cur || (!isset($_GET['order_by']) && 'Time' === $Cur)) {
...
<?php    foreach ($WayVals as $Cur) { ?>
              <option value="<?=$Cur?>"<?php if (isset($_GET['order_way']) && $_GET['order_way'] === $Cur || (!isset($_GET['order_way']) && 'Descending' === $Cur)) {
    echo ' selected="selected"';
} ?>><?=$Cur?></option>
<?php    } ?>
            </select>

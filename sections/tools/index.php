...
} elseif (empty($_REQUEST['action']) || ('public_sandbox' != $_REQUEST['action'] && 'ocelot' != $_REQUEST['action'])) {
...
if ('sandbox' == substr($_REQUEST['action'], 0, 7) && !isset($argv[1]) && !check_perms('site_debug')) {
    error(403);
}

if ('update_geoip' == substr($_REQUEST['action'], 0, 12) && !isset($argv[1]) && !check_perms('site_debug')) {
    error(403);
}

if ('rerender_gallery' == substr($_REQUEST['action'], 0, 16) && !isset($argv[1]) && !check_perms('site_debug')) {
    error(403);
}
...
switch ($_REQUEST['action']) {
    ...
    case 'deletenews':
        ...
        WHERE ID = '" . db_string($_GET['id']) . "'");
            ...
            if (false !== $LatestNews && $LatestNews == $_GET['id']) {
        ...
        if (!empty($_REQUEST['id'])) {
          ...
          GROUP BY p.ID");
            ...
            WHERE Level = '" . db_string($_REQUEST['level']) . "'");
                ...
                $DisplayStaff = $_REQUEST['displaystaff'] ?? 0;
              ...
              VALUES ('" . db_string($Level) . "',
                  ...
                  {$Secondary},
                  '" . db_string($Forums) . "',
                  '" . db_string(serialize(base64_decode($Values))) . "',

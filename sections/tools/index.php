...
if (isset($argv[1])) {
    ...
    if (empty($_REQUEST['action']) || ('public_sandbox' != $_REQUEST['action'] && 'ocelot' != $_REQUEST['action'])) {
...
if ('sandbox' == substr($_REQUEST['action'], 0, 7) && !isset($argv[1])) {
    if (!check_perms('site_debug')) {
        error(403);
    }
}

if ('update_geoip' == substr($_REQUEST['action'], 0, 12) && !isset($argv[1])) {
    if (!check_perms('site_debug')) {
        error(403);
    }
}

if ('rerender_gallery' == substr($_REQUEST['action'], 0, 16) && !isset($argv[1])) {
    if (!check_perms('site_debug')) {
        error(403);
    }
}
  ...
  case 'permissions':
    ...
    if (!empty($_REQUEST['id'])) {
        ...
        if (is_numeric($_REQUEST['id'])) {
          ...
          GROUP BY p.ID");
            [$ID, $Name, $Abbreviation, $Level, $Secondary, $Forums, $Values, $DisplayStaff, $UserCount] = $DB->next_record(MYSQLI_NUM, [6]);

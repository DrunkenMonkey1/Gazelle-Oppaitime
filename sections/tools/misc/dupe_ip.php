    ...
    <table width="100%">
        ...
        while (list($UserID, $IP, $Username, $PermissionID, $Enabled, $Donor, $Warned, $Joined, $Uses) = $DB->next_record()) {
            ...
            <tr class="row<?= $Row ?>">
                ...
                <td>
                    <span class="float_left"><?= Tools::get_host_by_ajax($IP) . " ($IP)" ?></span><span class="float_right"><a
                                href="userhistory.php?action=ips&amp;userid=<?= $UserID ?>"
                                title="History"
                                class="brackets tooltip">H</a> <a href="user.php?action=search&amp;ip_history=on&amp;ip=<?= display_str($IP) ?>"
                                                                  title="Search"
                                                                  class="brackets tooltip">S</a></span>
                </td>

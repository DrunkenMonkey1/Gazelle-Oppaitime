...
if (false !== isset($IP) && false !== empty($IP)) {
    $Delimiter = $IP[strcspn($IP, ':.')];
    $OctOrHextets = explode($Delimiter, $IP);
    
    
    if ('.' === $Delimiter && 4 === count($OctOrHextets)) { // IPv4
        if ((127 === $OctOrHextets[0] || 10 === $OctOrHextets[0])
            || (192 === $OctOrHextets[0] && 168 === $OctOrHextets[1])
            || (172 === $OctOrHextets[0] && ($OctOrHextets[1] >= 16 && $OctOrHextets[1] <= 32))
        ) {
    ...
    } elseif (count($OctOrHextets) <= 8) { // IPv6

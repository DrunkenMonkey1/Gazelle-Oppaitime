...
if (!Forums::check_forumperm((int) $ForumID)) {
    error(403);
}
...
<div class="thin">
    ...
    <?php if (check_perms('site_moderate_forums')) { ?>
        <div class="linkbox">
            <a href="forums.php?action=edit_rules&amp;forumid=<?= $ForumID ?>" class="brackets">Change specific
                rules</a>
        </div>
    <?php } ?>
            ...
            foreach ($Forum as $Topic) {
                ...
                if ($TopicPages > 1) {
                    ...
                    for ($i = 1; $i <= $TopicPages; ++$i) {
                        ...
                        $PageLinks[] = sprintf('<a href="forums.php?action=viewthread&amp;threadid=%s&amp;page=%s">%s</a>',
                            $TopicID, $i, $i);
                    }
...
$DisplayTitle = $Title; ?>
        <strong>
          <a href="forums.php?action=viewthread&amp;threadid=<?= $TopicID ?>"
             class="tooltip"
             data-title-plain="<?= $Title ?>"><?= Format::cut_string($DisplayTitle, $TopicLength) ?></a>
        </strong>
        <?= $PagesText ?>
      </span>
                        <?php if (!empty($LastRead[$TopicID])) { ?>
                            <a class="<?= $Tooltip ?> last_read"
        ...
        by <?= Users::format_username($LastAuthorID, false, false, false, false, false) ?> <?= time_diff($LastTime,
                                1) ?>
      </span>
                    </td>
  ...
  <a href="forums.php">Forums</a> &gt; <?= $ForumName ?>
    ...
    <div class="linkbox">
        <a href="forums.php?action=catchup&amp;forumid=<?= $ForumID ?>&amp;auth=<?= $LoggedUser['AuthKey'] ?>"

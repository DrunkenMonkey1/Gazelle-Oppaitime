...
$IsDonorForum = (DONOR_FORUM == $ForumID);
...
if (false === $QuoteNotificationsCount || $QuoteNotificationsCount > 0) {
    $DB->query("
...
<div class="thin">
  ...
  <div class="linkbox">
    <div class="center">
      ...
      <a href="#" onclick="Subscribe(<?=$ThreadID?>);return false;" id="subscribelink<?=$ThreadID?>" class="brackets"><?=(in_array($ThreadID, $UserSubscriptions, true) ? 'Unsubscribe' : 'Subscribe')?></a>
...
if (0 == $ThreadInfo['NoPoll']) {
    if (![$Question, $Answers, $Votes, $Featured, $Closed] = $Cache->get_value("polls_$ThreadID")) {
        $DB->query("
  ...
  <div class="box thin clear">
    <div class="head colhead_dark"><strong>Poll<?php if ($Closed) {
        ...
        echo ' [Featured]';
    } ?></strong> <a href="#" onclick="$('#threadpoll').gtoggle(); log_hit(); return false;" class="brackets">View</a></div>
    <div class="pad<?php if (/*$LastRead !== null || */$ThreadInfo['IsLocked']) {
        echo ' hidden';
    } ?>" id="threadpoll">
      <p><strong><?=display_str($Question)?></strong></p>
<?php  if (null !== $UserResponse || $Closed || $ThreadInfo['IsLocked'] || !Forums::check_forumperm($ForumID)) { ?>
...
<?php
        ...
        if ($Votes[0] > 0) {
            ?>
        <li><?=('0' == $UserResponse ? '&raquo; ' : '')?>(Blank) (<?=number_format((float)($Votes[0] / $TotalVotes * 100), 2)?>%)</li>
        <li class="graph">
...
<?php
        ...
        <li>
          <a href="forums.php?action=change_vote&amp;threadid=<?=$ThreadID?>&amp;auth=<?=$LoggedUser['AuthKey']?>&amp;vote=0"><?=('0' == $UserResponse ? '&raquo; ' : '')?>Blank</a> - <?=$StaffVotes[0]?>&nbsp;(<?=number_format(((float)$Votes[0] / $TotalVotes) * 100, 2)?>%)
        </li>
      </ul>
<?php

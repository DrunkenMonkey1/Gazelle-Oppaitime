...
if (!empty($_REQUEST['confirm'])) {
    ...
    $Val->SetFields('password', true, 'regex', 'Your password must be at least 6 characters long.',
        ['regex' => '/(?=^.{6,}$).*$/']);
    $Val->SetFields('confirm_password', true, 'compare', 'Your passwords do not match.',
    ...
    $Val->SetFields('agereq', true, 'checkbox',
        'You did not select the box that says you are 18 years of age or older.');
    ...
    if (!empty($_POST['submit'])) {
        ...
        */
        if (!$Err) {
            ...
            if ('0' == trim($_POST['username']) || '1' == trim($_POST['username'])) {
        ...
        WHERE Username LIKE '" . db_string(trim($_POST['username'])) . "'");
          ...
          WHERE InviteKey = '" . db_string($_REQUEST['invite']) . "'");
                if (!$DB->has_results()) {
                    ...
                    [$InviterID, $InviteEmail, $InviteReason] = $DB->next_record(MYSQLI_NUM, false);
        ...
        VALUES
          ('" . db_string(trim($_POST['username'])) . "', '" . Crypto::encrypt($_POST['email']) . "', '" . db_string(Users::make_sec_hash($_POST['password'])) . "', '" . db_string($torrent_pass) . "', '" . Crypto::encrypt($_SERVER['REMOTE_ADDR']) . sprintf('\', \'%s\', \'%s\', \'',
                    $Class, $Enabled) . STARTING_INVITES . sprintf('\', \'1073741824\', \'%s\')', $IPcc));
        ...
        WHERE InviteKey = '" . db_string($_REQUEST['invite']) . "'");
            ...
            if (Badges::award_badge($InviterID, 136)) {
                Misc::send_pm($InviterID, 0, 'You have received a badge!',
                    "You have received a badge for inviting a user to the site.\n\nIt can be enabled from your user settings.");
        ...
        FROM stylesheets
        WHERE `Default` = '1'");
            [$StyleID] = $DB->next_record();
            $AuthKey = Users::make_secret();
            
            if ('' !== $InviteReason) {
                $InviteReason = db_string(sqltime() . sprintf(' - %s', $InviteReason));
            }
            $DB->query("
        INSERT INTO users_info
          (UserID, StyleID, AuthKey, Inviter, JoinDate, AdminComment)
        VALUES
          ('{$UserID}', '{$StyleID}', '" . db_string($AuthKey) . sprintf('\', \'%s\', NOW(), \'%s\')', $InviterID,
                    $InviteReason));
        ...
        VALUES
          ...
          VALUES
            ...
            if (null !== $InviterID && 0 !== $InviterID) {
                $DB->query("
          SELECT TreePosition, TreeID, TreeLevel
          FROM invite_tree
          WHERE UserID = '{$InviterID}'");
                [$InviterTreePosition, $TreeID, $TreeLevel] = $DB->next_record();
    ...
    include __DIR__ . '/step1.php';
} elseif (!OPEN_REGISTRATION) {
    if (isset($_GET['welcome'])) {
        include __DIR__ . '/code.php';
    } else {
        include __DIR__ . '/closed.php';
    }
}

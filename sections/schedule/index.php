...
if ((!isset($_REQUEST['key']) || SCHEDULE_KEY != $_REQUEST['key']) && (!isset($argv[1]) || SCHEDULE_KEY != $argv[1]) && !$AS) {
    error(403);
}
...
$DB->query("
  ...
  FROM schedule");
[$Hour, $Day, $BiWeek] = $DB->next_record();
...
//-------------- Run every week -------------------------------------------//
if (($Day != $NextDay && 0 == date('w')) || (isset($_GET['runweek']) && $_GET['runweek'])) {
    run_all_in('weekly');
...
//---------------- Run every month -----------------------------------------//
if (($BiWeek != $NextBiWeek && 8 == $BiWeek) || (isset($_GET['runmonth']) && $_GET['runmonth'])) {
    run_all_in('monthly');

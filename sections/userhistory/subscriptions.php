...
?>
    <div class="thin">
        <div class="header">
            <h2>
                Subscriptions<?= $ShowUnread ? ' with unread posts' . ($NumResults ? ' (' . $NumResults . ' new)' : '') : '' ?></h2>
            ...
            <div class="linkbox">
                ...
                if (!$ShowUnread) {
                    ?>
                    <br/><br/>
                    <a href="userhistory.php?action=subscriptions&amp;showunread=1" class="brackets">Only display
                        subscriptions with unread replies</a>&nbsp;&nbsp;&nbsp;
                    <?php
                } else {
                    ?>
                    <br/><br/>
                ...
                if ($NumResults) {
                    ?>
                    <a href="#"
                       onclick="Collapse(); return false;"
                       id="collapselink"
                       class="brackets"><?= $ShowCollapsed ? 'Show' : 'Hide' ?> post bodies</a>&nbsp;&nbsp;&nbsp;
                    <?php
                }
                ?>
                <a href="userhistory.php?action=catchup&amp;auth=<?= $LoggedUser['AuthKey'] ?>" class="brackets">Catch
                    up</a>&nbsp;&nbsp;&nbsp;
                <a href="userhistory.php?action=posts&amp;userid=<?= $LoggedUser['ID'] ?>" class="brackets">Go to post
                    history</a>&nbsp;&nbsp;&nbsp;
            ...
            foreach ($Results as $Result) {
                switch ($Result['Page']) {
                    ...
                    case 'requests':
                        ...
                        $Links = 'Request: ';
                        if ('Music' == $CategoryName || 'Audiobooks' == $CategoryName || 'Comedy' == $CategoryName) {
                            $Links .= ('Music' == $CategoryName ? Artists::display_artists(Requests::get_artists($Result['PageID'])) : '') . '<a href="requests.php?action=view&amp;id=' . $Result['PageID'] . '" dir="ltr">' . $Request['Title'] . " [" . $Request['Year'] . "]</a>";
                        } else {
          ...
          <?= $Links . ($Result['PostID'] < $Result['LastPost'] ? ' <span class="new">(New!)</span>' : '') ?>
                            ...
                            <a class="tooltip last_read" title="Jump to last read" href="<?= $JumpLink ?>">
                                <svg width="15" height="11">
                                    <polygon points="0,3 0,8 8,8 8,11 15,5.5 8,0 8,3"/>
                                </svg>
                            </a>
                            <?php if ('forums' == $Result['Page']) { ?>
                        ...
                        <tr class="row<?= $ShowCollapsed ? ' hidden' : '' ?>">
                            <?php if (Users::has_avatars_enabled()) { ?>
                                <td class="avatar" valign="top">
                                    <?= Users::show_avatar($Result['LastReadAvatar'], $Result['LastReadUserID'],
                                        $Result['LastReadUsername'], $HeavyInfo['DisableAvatars']) ?>
                                </td>
                            <?php } ?>
                            <td class="body" valign="top">
                                <div class="content3">
                                    ...
                                    <?php if ($Result['LastReadEditedUserID']) { ?>
                                        <br/><br/>
                                        Last edited by <?= Users::format_username($Result['LastReadEditedUserID'],
                                            false, false, false) ?> <?= time_diff($Result['LastReadEditedTime']) ?>
                                    <?php } ?>
                                </div>
                            </td>
                        </tr>

...
if (!empty($_GET['revisionid'])) { // if they're viewing an old revision
    ...
    $Data = $Cache->get_value(sprintf('artist_%s_revision_%s', $ArtistID, $RevisionID), true);
} else { // viewing the live version
    $Data = $Cache->get_value(sprintf('artist_%s', $ArtistID), true);
    $RevisionID = false;
}
...
if (($GroupIDs = $Cache->get_value(sprintf('artist_groups_%s', $ArtistID))) === false) {
    $DB->query("
    SELECT
    ...
    $Cache->cache_value(sprintf('artist_groups_%s', $ArtistID), $GroupIDs, 0);
}
...
<div id="discog_table" class="box">
    ...
    $UnknownRT = array_search('Unknown', $ReleaseTypes, true);
    if (false === $UnknownRT) {
        $UnknownRT = 1025;
        $ReleaseTypes[$UnknownRT] = 'Unknown';
    }
    
    $SortOrder = empty($LoggedUser['SortHide']) ? $ReleaseTypes : array_flip(array_keys($LoggedUser['SortHide']));
    ...
    $OpenTable = false;
    $ShowGroups = !isset($LoggedUser['TorrentGrouping']) || 0 == $LoggedUser['TorrentGrouping'];
    ...
    <table class="torrent_table grouped release_table">
        ...
        foreach ($TorrentList as $Group) {
            ...
            if (count($Torrents) > 1) {
                ...
                $DisplayName .= sprintf('<a href="torrents.php?id=%s" class="tooltip" title="View torrent group" ',
                    $GroupID);
              ...
              <a class="float_right"
                ...
                foreach ($Torrents as $TorrentID => $Torrent) {
                    ...
                    if (!empty(G::$LoggedUser) && (G::$LoggedUser['ShowMagnets'] ?? false)) {
                        $TorrentMG = "magnet:?xt=urn:btih:" . $Torrent['info_hash'] . "&as=https://" . SITE_DOMAIN . "/" . str_replace('&amp;',
                                '%26',
                                $TorrentDL) . "&tr=" . implode("/" . $LoggedUser['torrent_pass'] . "/announce&tr=",
                                ANNOUNCE_URLS[0]) . "/" . $LoggedUser['torrent_pass'] . "/announce&xl=" . $Torrent['Size'];
                    } ?>
...
<?php } ?>
          | <a href="reportsv2.php?action=report&amp;id=<?= $TorrentID ?>" class="tooltip" title="Report">RP</a> ]
                        ...
                        <td class="number_column<?= ((0 == $Torrent['Seeders']) ? ' r00' : '') ?>"><?= number_format($Torrent['Seeders']) ?></td>
            ...
            <a href="#"
               ...
               onclick="Bookmark('torrent', <?= $GroupID ?>, 'Remove bookmark'); return false;">Bookmark</a>
                            ...
                            <div style="display: inline;"
                                 class="torrent_info"><?= $ExtraInfo ?><?php if ($Reported) { ?> /
                                    <strong class="torrent_label tl_reported">Reported</strong><?php } ?></div>
                            <div class="tags"><?= $TorrentTags->format('torrents.php?taglist=', $Name) ?></div>
                        </div>
                    </td>
                    <td class="number_column nobr"><?= Format::get_size($Torrent['Size']) ?></td>
...
<div class="thin">
    <div class="header">
        <h2><?= display_str($Name) ?><?php if ($RevisionID) { ?> (Revision #<?= $RevisionID ?>)<?php } ?></h2>
        <div class="linkbox">
      ...
      LIMIT 1");
                ...
                if (!is_null($Notify['Artists']) && false === stripos($Notify['Artists'], sprintf('|%s|', $Name))) {
                    ?>
            ...
            if (Bookmarks::has_bookmarked('artist', $ArtistID)) {
                ?>
                <a href="#"
                   id="bookmarklink_artist_<?= $ArtistID ?>"
            ...
            <?php } ?>
            <a href="#"
               id="subscribelink_artist<?= $ArtistID ?>"
               class="brackets"
               onclick="SubscribeComments('artist', <?= $ArtistID ?>);return false;"><?= false !== Subscriptions::has_subscribed_comments('artist',
                    $ArtistID) ? 'Unsubscribe' : 'Subscribe' ?></a>
    ...
    <div class="main_column">
        ...
        $Collages = $Cache->get_value(sprintf('artists_collages_%s', $ArtistID));
    ...
    WHERE ca.ArtistID = '{$ArtistID}'
      ...
      AND CategoryID = '7'");
            ...
            $Cache->cache_value(sprintf('artists_collages_%s', $ArtistID), $Collages, 3600 * 6);
        }
        if (count($Collages) > 0) {
            ...
            <table class="collage_table" id="collages">
                <tr class="colhead">
                    <td width="85%"><a href="#">&uarr;</a>&nbsp;This artist is in <?= number_format(count($Collages)) ?>
                        collage<?= ((count($Collages) > 1) ? 's' : '') ?><?= $SeeAll ?></td>
                    <td># artists</td>
                </tr>
                <?php
        ...
        if ($NumRequests > 0) {
            ...
            <table cellpadding="6" cellspacing="1" border="0" class="request_table border" width="100%" id="requests">
                ...
                foreach ($Requests as $RequestID => $Request) {
                    ...
                    $FullName = $ArtistLink . sprintf('<a href="requests.php?action=view&amp;id=%s"><span dir="ltr">%s</span></a>',
                            $RequestID, $Title);
                    
                    if ($Request['CatalogueNumber']) {
                        $FullName .= sprintf(' [%s]', $Request[CatalogueNumber]);
                    }
                    if ($Request['DLSiteID']) {
                        $FullName .= sprintf(' [%s]', $Request[DLSiteID]);
                    }
                    
                    if (!empty($Tags[$RequestID])) {
                        $ReqTagList = [];
                        foreach ($Tags[$RequestID] as $TagID => $TagName) {
                            $ReqTagList[] = sprintf('<a href="requests.php?tags=%s">',
        ...
        <div id="artistcomments">
            ...
            //---------- Begin printing
            CommentsView::render_comments($Thread, $LastRead, sprintf('artist.php?id=%s', $ArtistID));
            ?>
            <div class="linkbox">
                <?= ($Pages) ?>
            </div>

...
function html_entity_decode (str) {
    ...
    el.innerHTML = str;
    for (let i = 0, ret = ''; i < el.childNodes.length; i++) {
...
function get_size (size) {
    ...
    const exts = ['B', 'KiB', 'MiB', 'GiB', 'TiB', 'PiB', 'EiB', 'ZiB', 'YiB'];
    return (size.toFixed(2) + (exts[steps] || ''));
}

function ratio (a, b) {
    let rc = 'r50';
    for (const i of [[5, '20'], [2, '10'], [1, '09'], [0.9, '08'], [0.8, '07'], [0.7, '06'],
...
$.fn.extend({
    ...
    remove_class: function (class_name) {
        for (let i = 0; i < this.length; i++) {
            ...
            const result = classes.indexOf(class_name);
            if (result != -1) {
    ...
    toggle_class: function (class_name) {
        for (let i = 0, il; i < this.length; i++) {
            const object = this[i];
            const classes = object.className.split(' ');
            const result = classes.indexOf(class_name);
            if (result != -1) {
...
if ($('meta[name=authkey]').raw()) {
    const authkey = $('meta[name=authkey]').raw().content;
    const userid = parseInt($('meta[name=userid]').raw().content);
}
...
function add_selection () {
    const selected = $('#formats').raw().options[$('#formats').raw().selectedIndex];

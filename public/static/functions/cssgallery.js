$(document).ready(function () {
    ...
    if (!$('input#styleurl').val()) {
        const radiobutton = $('input[name="stylesheet_gallery"][value="' + $('select#stylesheet').val() + '"]');
        radiobutton.click();
    ...
    $('input[name="stylesheet_gallery"]').click(function () {
        ...
        const radiobutton = parent.find('input');
    ...
    $('select#stylesheet').change(function () {
        const radiobutton = $('input[name="stylesheet_gallery"][value="' + $(this).val() + '"]');
        radiobutton.prop('checked', true);
        $('.preview_wrapper').removeClass('selected');
        radiobutton.parent().parent().addClass('selected');
        $('input#styleurl').val('');
    ...
    const userstyle = $('link[rel=stylesheet][title]')[0];
    $('select#stylesheet').on('change', function (e) {
        const changetext = e.target.options[e.target.selectedIndex].text.toLowerCase();

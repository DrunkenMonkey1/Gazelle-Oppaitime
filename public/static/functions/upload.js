...
function Categories () {
    ...
    let cats = [{
        ...
        'screenshots': {
            name: 'Screenshots',
            notes: '<strong class="important_text">Promotional materials from a game\'s store page are NOT screenshots</strong>'
        },
...
function MediaInfoExtract () {
    ...
    ['container', 'codec', 'resolution', 'audioformat', 'lang'].forEach((sel) => {
        if (sel == 'resolution') {
            ...
            if (width == '680' && height == '480') { ressel.value = 'SD'; } else if (height == '480') { ressel.value = '480p'; } else if (height == '720') { ressel.value = '720p'; } else if (height == '1080' && getval(mi, 'Scan type' == 'Interlaced')) { ressel.value = '1080i'; } else if (height == '1080') { ressel.value = '1080p'; } else if (width == '3840') { ressel.value = '4K'; } else {
                ressel.value = 'Other';
